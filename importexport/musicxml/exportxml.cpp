...
static double accSymId2alter(SymId id)
{
    ...
    break;
    case SymId::accidentalThreeQuarterTonesFlatZimmermann: {
        res = -1.5;
    }
    break;
    ...
    break;
    case SymId::accidentalThreeQuarterTonesSharpStein: {
        res = 1.5;
    }
    break;
...
void ExportMusicXml::keysig(const KeySig* ks, ClefType ct, int staff, bool visible)
{
    ...
    if (kse.custom() && !kse.isAtonal() && keysyms.size() > 0) {
        ...
        break;
        case KeyMode::DORIAN: {
            _xml.tag("mode", "dorian");
        }
        break;
        case KeyMode::PHRYGIAN: {
            _xml.tag("mode", "phrygian");
        }
        break;
        case KeyMode::LYDIAN: {
            _xml.tag("mode", "lydian");
        }
        break;
        case KeyMode::MIXOLYDIAN: {
            _xml.tag("mode", "mixolydian");
        }
        break;
        case KeyMode::AEOLIAN: {
            _xml.tag("mode", "aeolian");
        }
        break;
        case KeyMode::IONIAN: {
            _xml.tag("mode", "ionian");
        }
        break;
        case KeyMode::LOCRIAN: {
            _xml.tag("mode", "locrian");
        }
        break;
...
static void tremoloSingleStartStop(Chord* chord, Notations& notations, Ornaments& ornaments, XmlWriter& xml)
{
    if (chord->tremolo()) {
        ...
        if (chord->tremoloChordType() == TremoloChordType::TremoloSingle) {
            ...
            break;
            default: {
                qDebug("unknown tremolo single %d", int(st));
            }
            break;
            }
            ...
            break;
            default: {
                qDebug("unknown tremolo double %d", int(st));
            }
            break;
            }
