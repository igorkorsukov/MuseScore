...
#if 0 // TODO-ws
            case 1: {
                addArticulationText(score, chord, track, QString("staccato"));
            }
            break;
            case 2: {
                addArticulationText(score, chord, track, QString("tenuto"));
            }
            break;
            case 3: {
                addArticulationText(score, chord, track, QString("portato"));
            }
            break;
            case 4: {
                addArticulationText(score, chord, track, QString("staccatissimo"));
            }
            break;
            case 5: {
                addArticulationText(score, chord, track, QString("sforzato"));
            }
            break;
            case 6: {
                addArticulationText(score, chord, track, QString("marcato"));
            }
            break;
#endif
            case 7:                     // "weak beat"
...
void NotelinesObj::read()
{
    ...
    switch (b) {
    case 1: { }
    break;                 // Einlinienzeile
    case 2: { }
    break;                 // Standard (5 Linien)
    default: {
        Q_ASSERT(b == 0);
        char lines[11];
        cap->read(lines, 11);
...
ClefType CapClef::clefType(Form form, ClefLine line, Oct oct)
{
    ...
    switch (idx) {
    case int(Form::G) + (int(ClefLine::L2) << 3) + (int(Oct::OCT_NULL) << 5): {
        return ClefType::G;
    }
    case int(Form::G) + (int(ClefLine::L2) << 3) + (int(Oct::OCT_ALTA) << 5): {
        return ClefType::G8_VA;
    }
    case int(Form::G) + (int(ClefLine::L2) << 3) + (int(Oct::OCT_BASSA) << 5): {
        return ClefType::G8_VB;
    }
    case int(Form::C) + (int(ClefLine::L1) << 3) + (int(Oct::OCT_NULL) << 5): {
        return ClefType::C1;
    }
    case int(Form::C) + (int(ClefLine::L2) << 3) + (int(Oct::OCT_NULL) << 5): {
        return ClefType::C2;
    }
    case int(Form::C) + (int(ClefLine::L3) << 3) + (int(Oct::OCT_NULL) << 5): {
        return ClefType::C3;
    }
    case int(Form::C) + (int(ClefLine::L4) << 3) + (int(Oct::OCT_NULL) << 5): {
        return ClefType::C4;
    }
    case int(Form::C) + (int(ClefLine::L5) << 3) + (int(Oct::OCT_NULL) << 5): {
        return ClefType::C5;
    }
    case int(Form::F) + (int(ClefLine::L4) << 3) + (int(Oct::OCT_NULL) << 5): {
        return ClefType::F;
    }
    case int(Form::F) + (int(ClefLine::L4) << 3) + (int(Oct::OCT_BASSA) << 5): {
        return ClefType::F8_VB;
    }
    case int(Form::F) + (int(ClefLine::L3) << 3) + (int(Oct::OCT_NULL) << 5): {
        return ClefType::F_B;
    }
    case int(Form::F) + (int(ClefLine::L5) << 3) + (int(Oct::OCT_NULL) << 5): {
        return ClefType::F_C;
    }
    default: {
