            ...
            if (std::get<2>(_oldItemInfo) == row && std::get<1>(_oldItemInfo) == mouseOverArrow) {
                ...
                if (std::get<0>(_oldItemInfo)) {
                    std::pair<QGraphicsItem*,
                              int> p = std::make_pair(std::get<0>(_oldItemInfo), std::get<2>(_oldItemInfo));
                ...
                std::tuple<QGraphicsPixmapItem*, MouseOverValue, unsigned> tmp(graphicsPixmapItemArrow, mouseOverArrow,
                                                                               row);
                _oldItemInfo = tmp;
                if (mouseOverArrow != MouseOverValue::OPEN_EYE && mouseOverArrow != MouseOverValue::CLOSED_EYE) {
    ...
    QGraphicsView(parent)
{
    ...
    std::tuple<QString, void (Timeline::*)(Segment*, int*, int), bool> t2(tr("Time Signature"), &Ms::Timeline::timeMeta,
                                                                          true);
    std::tuple<QString, void (Timeline::*)(Segment*, int*, int), bool> t3(tr("Rehearsal Mark"),
                                                                          &Ms::Timeline::rehearsalMeta, true);
    std::tuple<QString, void (Timeline::*)(Segment*, int*, int), bool> t4(tr("Key Signature"), &Ms::Timeline::keyMeta,
                                                                          true);
    std::tuple<QString, void (Timeline::*)(Segment*, int*, int), bool> t5(tr("Barlines"), &Ms::Timeline::barlineMeta,
                                                                          true);
    std::tuple<QString, void (Timeline::*)(Segment*, int*, int), bool> t6(tr("Jumps and Markers"),
                                                                          &Ms::Timeline::jumpMarkerMeta, true);
    std::tuple<QString, void (Timeline::*)(Segment*, int*, int), bool> t7(tr("Measures"), &Ms::Timeline::measureMeta,
                                                                          true);
    _metas.push_back(t1);
    _metas.push_back(t2);
    _metas.push_back(t3);
...
void Timeline::drawGrid(int globalRows, int globalCols)
{
    ...
    for (int col = 0; col < globalCols; col++) {
        for (int row = 0; row < globalRows; row++) {
            ...
            graphicsRectItem->setToolTip(initialLetter + QString(" ") + QString::number(currMeasure->no() + 1) + QString(
                                             ", ") + partName);
...
void Timeline::rehearsalMeta(Segment* seg, int* stagger, int pos)
{
    ...
    for (Element* element : seg->annotations()) {
        ...
        if (element->isRehearsalMark()) {
            ...
            if (addMetaValue(x, pos, rehersal_mark->plainText(), row, ElementType::REHEARSAL_MARK, element, 0,
                             seg->measure())) {
                (*stagger)++;
                _globalZValue++;
            }
...
void Timeline::measureMeta(Segment*, int*, int pos)
{
    ...
    // Adjust number
    Measure* currMeasure;
    for (currMeasure = _score->firstMeasure(); currMeasureNumber != 0;
...
void Timeline::setMetaData(QGraphicsItem* gi, int staff, ElementType et, Measure* m, bool full_measure, Element* e,
                           QGraphicsItem* pairItem, Segment* seg)
{
    ...
    if (!hoveredGraphicsItem) {
        if (std::get<0>(_oldHoverInfo)) {
            ...
            QGraphicsRectItem* graphicsRectItem2
                = qgraphicsitem_cast<QGraphicsRectItem*>(static_cast<QGraphicsItem*>(std::get<0>(_oldHoverInfo)->data(5)
                                                                                     .value<void*>()));
