...
MasterScore* MuseScore::readScore(const QString& name)
{
    ...
    if (rv == Score::FileError::FILE_TOO_OLD || rv == Score::FileError::FILE_TOO_NEW
        || rv == Score::FileError::FILE_CORRUPTED) {
...
void MuseScore::exportFile()
{
    ...
    if (cs->masterScore()->fileInfo()->exists()) {
        ...
        if (lastSaveCopyDirectory.isEmpty()) {
            lastSaveCopyDirectory
                = set.value("lastSaveCopyDirectory", preferences.getString(PREF_APP_PATHS_MYSCORES)).toString();
        }
    ...
    if (!cs->isMaster()) {
        ...
        name = QString("%1/%2.%3").arg(saveDirectory).arg(cs->masterScore()->fileInfo()->completeBaseName()).arg(
            saveFormat);
    }
...
bool MuseScore::exportParts()
{
    ...
    if (ext.toLower() == "pdf") {
        ...
        QString partfn(fi.absolutePath() + "/" + fi.completeBaseName() + "-"
                       + createDefaultFileName(tr("Score_and_Parts")) + ".pdf");
        QFileInfo fip(partfn);
        if (fip.exists() && !overwrite) {
            if (!noToAll) {
                QMessageBox msgBox(QMessageBox::Question, confirmReplaceTitle,
...
{
    ExportMidi em(score);
    return em.write(name, preferences.getBool(PREF_IO_MIDI_EXPANDREPEATS), preferences.getBool(
                        PREF_IO_MIDI_EXPORTRPNS), synthesizerState());
}

bool MuseScore::saveMidi(Score* score, QIODevice* device)
{
    ExportMidi em(score);
    return em.write(device, preferences.getBool(PREF_IO_MIDI_EXPANDREPEATS), preferences.getBool(
                        PREF_IO_MIDI_EXPORTRPNS), synthesizerState());
}
    ...
    if (ret == QMessageBox::Yes) {
        ...
        if (!destPath.isEmpty()) {
            ...
            if (orig.copy(destFilePath)) {
                QMessageBox::information(0, QWidget::tr("SoundFont installed"),
                                         QWidget::tr(
...
Score::FileError readScore(MasterScore* score, QString name, bool ignoreVersionError)
{
    ...
    auto read
        = [](MasterScore* score, const QString& name, bool ignoreVersionError,
             bool imported = false)->Score::FileError {
    ...
    if (cs_->masterScore()->fileInfo()->exists()) {
        ...
        if (saveCopy) {
            if (mscore->lastSaveCopyDirectory.isEmpty()) {
                mscore->lastSaveCopyDirectory
                    = set.value("lastSaveCopyDirectory", preferences.getString(PREF_APP_PATHS_MYSCORES)).toString();
            }
            saveDirectory = mscore->lastSaveCopyDirectory;
        } else {
            if (mscore->lastSaveDirectory.isEmpty()) {
                mscore->lastSaveDirectory = set.value("lastSaveDirectory", preferences.getString(
                                                          PREF_APP_PATHS_MYSCORES)).toString();
            }
...
bool MuseScore::saveSelection(Score* cs_)
{
    if (!cs_->selection().isRange()) {
        if (!MScore::noGui) {
            QMessageBox::warning(mscore, tr("Save Selection"), tr("Please select one or more measures"));
        }
        return false;
    }
    QStringList fl;
...
bool MuseScore::exportTransposedScoreToJSON(const QString& inFilePath, const QString& transposeOptions,
                                            const QString& outFilePath)
{
    ...
    score->startCmd();
    const bool transposed = score->transpose(mode, direction, targetKey, transposeInterval, transposeKeySignatures,
                                             transposeChordNames, useDoubleSharpsFlats);
    if (!transposed) {
