    ...
    if (!verifyFileRes) {
        ...
        if (result) {
            ...
            QPushButton* uninstallButton
                = static_cast<QPushButton*>(extensionsTable->indexWidget(extensionsTable->model()->index(rowId, 4)));
...
void ResourceManager::uninstallExtension()
{
    ...
    if (mscore->uninstallExtension(extensionId)) {
        ...
        QPushButton* installButton
            = static_cast<QPushButton*>(extensionsTable->indexWidget(extensionsTable->model()->index(rowId, 3)));
