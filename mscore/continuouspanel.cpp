...
void ContinuousPanel::paint(const QRect&, QPainter& painter)
{
    ...
    for (const Element* e : el) {
        ...
        if (e->isStaffLines()) {
            ...
            if (staffName == "") {
                QList<StaffName>& staffNamesShort
                    = currentStaff->part()->instrument(Fraction::fromTicks(tick))->shortNames();
            ...
            if (currentTimeSig) {
                ...
                newTs->setSig(Fraction(_currentTimeSig.numerator(), _currentTimeSig.denominator()),
                              TimeSigType::NORMAL);
            }
