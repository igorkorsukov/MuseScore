...
void Seq::stop()
{
    ...
    if (!preferences.getBool(PREF_IO_JACK_USEJACKTRANSPORT)
        || (preferences.getBool(PREF_IO_JACK_USEJACKTRANSPORT) && _driver->getState() == Transport::PLAY)) {
        stopTransport();
    }
    if (cv) {
        ...
        case SeqMsgId::TEMPO_CHANGE:
        {
            ...
            if (playFrame != 0) {
                ...
                if (cachedPrefs.jackTimeBaseMaster && cachedPrefs.useJackTransport) {
                    _driver->seekTransport(utick + 2
                                           * cs->utime2utick(qreal((_driver->bufferSize()) + 1)
                                                             / qreal(MScore::sampleRate)));
                }
            } else {
                cs->tempomap()->setRelTempo(msg.realVal);
            }
            ...
            if (inCountIn) {
                ...
                if (mscore->loop()) {
                    ...
                    if (loopOutUTick < scoreEndUTick) {
                        ...
                        if (playPosUTick >= loopOutUTick - 2 * framesPerPeriodInTicks
                            || cs->repeatList().utick2tick(playPosUTick) < cs->loopInTick().ticks()) {
                            qDebug(
                                "Process: playPosUTick = %d, cs->loopInTick().ticks() = %d, cs->loopOutTick().ticks() = %d, getCurTick() = %d, loopOutUTick = %d, playFrame = %d",
                                playPosUTick,      cs->loopInTick().ticks(),
                                cs->loopOutTick().ticks(),      getCurTick(),      loopOutUTick,    *pPlayFrame);
                            if (cachedPrefs.useJackTransport) {
                                ...
                                if (loopInUTick != 0) {
                                    int seekto = loopInUTick - 2 * cs->utime2utick(
                                        (qreal)_driver->bufferSize() / MScore::sampleRate);
...
void Seq::setLoopOut()
{
    ...
    } else if (t > cs->lastMeasure()->endTick()) {
