...
void ScoreVersionListModel::update()
{
    ...
    if (_score) {
        ...
        if (_score->fileInfo() && _score->fileInfo()->isReadable()) {
            _versions.emplace_back(_score, tr("Last saved version"), ScoreVersion::INDEX_LAST_SAVED,
                                   *_score->fileInfo(), false);
        }
        if (_score->sessionStartBackupInfo().isReadable()) {
            _versions.emplace_back(_score, tr(
                                       "Session start"), ScoreVersion::INDEX_SESSION_START,
                                   _score->sessionStartBackupInfo(), false);
        }
    }
    endResetModel();
}
