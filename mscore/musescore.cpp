...
bool MuseScore::importExtension(QString path)
{
    ...
    if (!extensionsDirInfo.isWritable()) {
        if (!MScore::noGui) {
            QMessageBox::critical(mscore, QWidget::tr("Import Extension File"),
                                  QWidget::tr("Cannot import extension on read-only storage: %1").arg(extensionsPath));                                                     // TODO: on read-only *directory*
        }
        return false;
    }
    ...
    if (totalZipSize >= storage.bytesAvailable()) {
        if (!MScore::noGui) {
            QMessageBox::critical(mscore, QWidget::tr("Import Extension File"),
                                  QWidget::tr("Cannot import extension: storage %1 is full").arg(storage.displayName()));
        }
        return false;
    }
    ...
    QSet<QString> acceptableFolders
        = { Extension::sfzsDir, Extension::soundfontsDir, Extension::templatesDir, Extension::instrumentsDir,
            Extension::workspacesDir };
    for (auto fi : zipFile.fileInfoList()) {
    ...
    if (!hasMetadata) {
        if (!MScore::noGui) {
            QMessageBox::critical(mscore, QWidget::tr("Import Extension File"),
                                  QWidget::tr("Corrupted extension: no metadata.json"));
        }
        return false;
    }
    if (hasAlienDirectory) {
        if (!MScore::noGui) {
            QMessageBox::critical(mscore, QWidget::tr("Import Extension File"),
                                  QWidget::tr("Corrupted extension: unsupported directories in root directory"));
        }
        return false;
    }
    if (hasAlienFiles) {
        if (!MScore::noGui) {
            QMessageBox::critical(mscore, QWidget::tr("Import Extension File"),
                                  QWidget::tr("Corrupted extension: unsupported files in root directory"));
        }
        return false;
    }
    zipFile.close();
    ...
    if (extensionId.isEmpty() || version.isEmpty()) {
        if (!MScore::noGui) {
            QMessageBox::critical(mscore, QWidget::tr("Import Extension File"),
                                  QWidget::tr("Corrupted extension: corrupted metadata.json"));
        }
        return false;
    }
    ...
    if (dirList.contains(extensionId)) {
        ...
        if (versionDirList.size() > 0) {
            ...
            for (auto versionDir : versionDirList) {
                if (compareVersion(version, versionDir)) {
                    ...
                    if (!MScore::noGui) {
                        QMessageBox::critical(mscore, QWidget::tr("Import Extension File"),
                                              QWidget::tr("A newer version is already installed"));
                    }
                    newerVersion = true;
                    return false;
                }
        ...
        if (!newerVersion) {
            ...
            if (!d.removeRecursively()) {
                if (!MScore::noGui) {
                    QMessageBox::critical(mscore, QWidget::tr("Import Extension File"),
                                          QWidget::tr("Error while deleting previous version of the extension: %1").arg(
                                              extensionId));
                }
                return false;
            }
    ...
    QFuture<bool> futureUnzip
        = QtConcurrent::run(zipFile3, &MQZipReader::extractAll,
                            QString("%1/%2/%3").arg(preferences.getString(PREF_APP_PATHS_MYEXTENSIONS)).arg(
    ...
    if (!unzipResult) {
        if (!MScore::noGui) {
            QMessageBox::critical(mscore, QWidget::tr("Import Extension File"),
                                  QWidget::tr("Unable to extract files from the extension"));
        }
        return false;
    }
    ...
    auto loadSoundFontAsync = [&]() {
                                  ...
                                  QDir sfzDir(QString("%1/%2/%3/%4").arg(preferences.getString(
                                                                             PREF_APP_PATHS_MYEXTENSIONS)).arg(
                                                  extensionId).arg(version).arg(Extension::sfzsDir));
                                  if (sfzDir.exists()) {
                                      // get all sfz files
                                      QDirIterator it(sfzDir.absolutePath(), QStringList("*.sfz"), QDir::Files,
                                  ...
                                  QDir sfDir(QString("%1/%2/%3/%4").arg(preferences.getString(
                                                                            PREF_APP_PATHS_MYEXTENSIONS)).arg(
                                                 extensionId).arg(version).arg(Extension::soundfontsDir));
                                  if (sfDir.exists()) {
                                      // get all soundfont files
                                      QStringList filters("*.sf2");
                                      filters.append("*.sf3");
                                      QDirIterator it(sfDir.absolutePath(), filters, QDir::Files,
                                                      QDirIterator::Subdirectories);
                                      QStringList sfs;
                                      while (it.hasNext()) {
                                          it.next();
    ...
    QDir workspacesDir(QString("%1/%2/%3/%4").arg(preferences.getString(PREF_APP_PATHS_MYEXTENSIONS)).arg(extensionId).
                       arg(version).arg(Extension::workspacesDir));
...
bool MuseScore::uninstallExtension(QString extensionId)
{
    ...
    QDir sfzDir(QString("%1/%2/%3/%4").arg(preferences.getString(PREF_APP_PATHS_MYEXTENSIONS)).arg(extensionId).arg(
                    version).arg(Extension::sfzsDir));
    if (sfzDir.exists()) {
        // get all sfz files
    ...
    QDir sfDir(QString("%1/%2/%3/%4").arg(preferences.getString(PREF_APP_PATHS_MYEXTENSIONS)).arg(extensionId).arg(
                   version).arg(Extension::soundfontsDir));
    if (sfDir.exists()) {
        // get all soundfont files
        QStringList filters("*.sf2");
        filters.append("*.sf3");
    ...
    QMainWindow()
{
...

    {
        ...
        AccessibleToolButton* addWorkspaceButton
            = new AccessibleToolButton(workspacesTools, getAction("create-new-workspace"));
...
int MuseScore::appendScore(MasterScore* score)
{
    ...
    for (int i = 0; i < scoreList.size(); ++i) {
        if ((!score->importedFilePath().isEmpty()
            ...
            || (scoreList[i]->fileInfo()->canonicalFilePath() == score->fileInfo()->canonicalFilePath()
                && score->fileInfo()->exists())) {
            removeTab(i);
            index = i;
            break;
        }
...
bool MuseScore::processMidiRemote(MidiRemoteType type, int dta, int value)
{
    ...
    for (int i = 0; i < MIDI_REMOTES; ++i) {
        if (preferences.midiRemote(i).type == type && preferences.midiRemote(i).data == dta) {
            ...
            QAction* a = 0;
            switch (i) {
            case RMIDI_REWIND:      a = getAction("rewind");
                break;
            case RMIDI_TOGGLE_PLAY: a = getAction("play");
                break;
            case RMIDI_PLAY:
                a = getAction("play");
                if (a->isChecked()) {
            ...
            case RMIDI_STOP:
                ...
                break;
            case RMIDI_NOTE1:   a = getAction("pad-note-1");
                break;
            case RMIDI_NOTE2:   a = getAction("pad-note-2");
                break;
            case RMIDI_NOTE4:   a = getAction("pad-note-4");
                break;
            case RMIDI_NOTE8:   a = getAction("pad-note-8");
                break;
            case RMIDI_NOTE16:  a = getAction("pad-note-16");
                break;
            case RMIDI_NOTE32:  a = getAction("pad-note-32");
                break;
            case RMIDI_NOTE64:  a = getAction("pad-note-64");
                break;
            case RMIDI_REST:    a = getAction("rest");
                break;
            case RMIDI_DOT:     a = getAction("pad-dot");
                break;
            case RMIDI_DOTDOT:  a = getAction("pad-dotdot");
                break;
            case RMIDI_TIE:     a = getAction("tie");
                break;
            case RMIDI_UNDO:    a = getAction("undo");
                break;
            case RMIDI_NOTE_EDIT_MODE: a = getAction("note-input");
                break;
            case RMIDI_REALTIME_ADVANCE: a = getAction("realtime-advance");
                break;
            }
            if (a) {
                a->trigger();
        ...
        if (e->isTextBase()) {
            ...
            if (!(e->isFiguredBass() || e->isHarmony())) {        // do not show text tools for f.b.
                if (timelineScrollArea() && timelineScrollArea()->isVisible()) {
                    if (dockWidgetArea(timelineScrollArea()) != dockWidgetArea(textTools())
...
void MuseScore::writeSettings()
{
    ...
    if (cs) {
        ...
        for (int idx = 0; idx < scoreList.size(); ++idx) {
            settings.setValue(QString("score-%1").arg(idx),
                              scoreList[idx]->masterScore()->fileInfo()->absoluteFilePath());
        }
    }
        ...
        if (!name.isEmpty()) {
            ...
            if (!cs->loadStyle(name)) {
                QMessageBox::StandardButton b = QMessageBox::warning(this, tr("Load Style"),
                                                                     tr(
                                                                         "MuseScore may not be able to load this style file: %1").arg(
                                                                         MScore::lastError),
            ...
            for (auto i = il->begin(); i != il->end(); i++) {
                for (const Channel* channel : i->second->channel()) {
                    ...
                    for (MidiCoreEvent e : a->initList()) {
                        ...
                        int syntiIdx= synth->index(score->masterScore()->midiMapping(
                                                       a->channel())->articulation()->synti());
...
void MuseScore::updateUiStyleAndTheme()
{
    ...
    if (preferences.isThemeDark()) {
        css.replace("$buttonHighlightDisabledOff",
                    Ms::preferences.getColor(PREF_UI_BUTTON_HIGHLIGHT_COLOR_DISABLED_DARK_OFF).name().toLatin1());
        css.replace("$buttonHighlightDisabledOn",
                    Ms::preferences.getColor(PREF_UI_BUTTON_HIGHLIGHT_COLOR_DISABLED_DARK_ON).name().toLatin1());
        css.replace("$buttonHighlightEnabledOff",
                    Ms::preferences.getColor(PREF_UI_BUTTON_HIGHLIGHT_COLOR_ENABLED_DARK_OFF).name().toLatin1());
        css.replace("$buttonHighlightEnabledOn", Ms::preferences.getColor(
                        PREF_UI_BUTTON_HIGHLIGHT_COLOR_ENABLED_DARK_ON).name().toLatin1());
    } else {
        css.replace("$buttonHighlightDisabledOff",
                    Ms::preferences.getColor(PREF_UI_BUTTON_HIGHLIGHT_COLOR_DISABLED_LIGHT_OFF).name().toLatin1());
        css.replace("$buttonHighlightDisabledOn",
                    Ms::preferences.getColor(PREF_UI_BUTTON_HIGHLIGHT_COLOR_DISABLED_LIGHT_ON).name().toLatin1());
        css.replace("$buttonHighlightEnabledOff",
                    Ms::preferences.getColor(PREF_UI_BUTTON_HIGHLIGHT_COLOR_ENABLED_LIGHT_OFF).name().toLatin1());
        css.replace("$buttonHighlightEnabledOn",
                    Ms::preferences.getColor(PREF_UI_BUTTON_HIGHLIGHT_COLOR_ENABLED_LIGHT_ON).name().toLatin1());
    }

    qApp->setStyleSheet(css);

    QString style = QString("*, QSpinBox { font: %1pt \"%2\" } ")
                    .arg(QString::number(preferences.getInt(PREF_UI_THEME_FONTSIZE)),
                         preferences.getString(PREF_UI_THEME_FONTFAMILY))
                    + qApp->styleSheet();
...
MuseScoreApplication* MuseScoreApplication::initApplication(int& argc, char** argv)
{
...
#ifdef BUILD_CRASH_REPORTER
    {
        ...
        CrashReporter::Handler* crashHandler = new CrashReporter::Handler(
            QDir::tempPath(), true, CRASHREPORTER_EXECUTABLE);
...
MuseScoreApplication::CommandLineParseResult MuseScoreApplication::parseCommandLineArguments(MuseScoreApplication* app)
{
    ...
    parser.addOption(QCommandLineOption({ "o", "export-to" }, "Export to 'file'. Format depends on file's extension",
                                        "file"));
    parser.addOption(QCommandLineOption({ "r", "image-resolution" },
                                        "Use with '-o <file>.png'. Set output resolution for image export", "DPI"));
    parser.addOption(QCommandLineOption({ "T", "trim-image" },
    ...
    parser.addOption(QCommandLineOption({ "P", "export-score-parts" },
    ...
    parser.addOption(QCommandLineOption({ "b", "bitrate" }, "Use with '-o <file>.mp3', sets bitrate, in kbps",
                                        "bitrate"));
    ...
    if (parser.isSet("r")) {
        ...
        if (ok) {
            ...
            fprintf(stderr,
                    "PNG resolution value '%s' not recognized, using default setting from preferences instead.\n", qPrintable(
                        temp));
        }
    }
    if (parser.isSet("T")) {
...
//                  qDebug("set paper size from default printer");
    ...
    if (MScore::debugMode) {
        ...
        qDebug() << "  Available virtual geometry:" << screen->availableVirtualGeometry().x()
                 << screen->availableVirtualGeometry().y() << screen->availableVirtualGeometry().width() << "x"
                 << screen->availableVirtualGeometry().height();
        qDebug() << "  Available virtual size:" << screen->availableVirtualSize().width() << "x"
                 << screen->availableVirtualSize().height();
        qDebug() << "  Depth:" << screen->depth() << "bits";
        qDebug() << "  Geometry:" << screen->geometry().x() << screen->geometry().y() << screen->geometry().width()
        ...
        qDebug() << "  Physical size:" << screen->physicalSize().width() << "x" << screen->physicalSize().height()
                 << "mm";
        qDebug() << "  Refresh rate:" << screen->refreshRate() << "Hz";
        qDebug() << "  Size:" << screen->size().width() << "x" << screen->size().height();
        qDebug() << "  Virtual geometry:" << screen->virtualGeometry().x() << screen->virtualGeometry().y()
...
//---
    ...
    QObject::connect(mscore, &MuseScore::scoreStateChanged,
                     ActionEventObserver::instance(), &ActionEventObserver::setScoreState);
#endif
    ...
    if (!scoresOnCommandline && preferences.getBool(PREF_UI_APP_STARTUP_SHOWSTARTCENTER)
        && (!restoredSession || mscore->scores().size() == 0)) {
