...
class ExportLy
{
    ...
    enum voltatype {
        startending, endending, startrepeat, endrepeat, bothrepeat, doublebar, brokenbar, endbar, none
    };
    struct  voltareg {
        voltatype voltart;
        int barno;
...
void ExportLy::brackRegister(int brnumber, int bratype, int staffnr, bool start, bool end)
{
    ...
    case BRACKET_NORMAL: {
        if (start) {
            lybracks[staffnr].brakstart = true;
        }
        if (end) {
            lybracks[staffnr].brakend = true;
        }
    ...
    case BRACKET_BRACE: {
        if (start) {
            lybracks[staffnr].bracestart = true;
        }
        if (end) {
            lybracks[staffnr].braceend = true;
        }
                ...
                if (st) {
                    for (int braclev = 0; braclev < st->bracketLevels(); braclev++) { // run thru bracketlevels of staff
                        if (st->bracket(braclev) != NO_BRACKET) { // if bracket
                            groupnumber++;
                            if (groupnumber < MAXPARTGROUPS) { // brackRegister(bracketnumber, brackettype, staffnr, start, end)
                                brackRegister(groupnumber, st->bracket(braclev), partnumber, true, false);
                                brackRegister(groupnumber,st->bracket(braclev), partnumber - 1 + st->bracketSpan(
                                                  braclev), false, true);
                            }
                        }// end of if bracket
...
int ExportLy::checkJumpOrMarker(int mnum, bool start, Element*& moj)
{
    ...
    if (start) {
        mnum--;      // we place these things at the end of the previous measure
    }
    cout << "mnum: " << mnum << "\n";
...
void ExportLy::findMarkerAtMeasureStart(Measure* m)
{
    for (auto ci = m->el()->begin(); ci != m->el()->end(); ++ci) {
        Element* dir = *ci;
        int tp = dir->type();
        if (tp == Element::MARKER) { // only markers, not jumps, are used at measure start.
            Marker* ma = (Marker*)dir;
...
int ExportLy::voltaCheckBar(Measure* meas, int i)
{
    ...
    if (meas->repeatFlags() == RepeatStart) {
        ...
        if ((voltarray[i].barno != taktnr - 1) and (voltarray[i].voltart != startrepeat) and (voltarray[i].voltart
                                                                                              != bothrepeat)) {
            i++;
            voltarray[i].voltart = startrepeat;
...
void ExportLy::findVolta()
{
    ...
    for (MeasureBase* m = score->first(); m; m = m->next()) {// for all measures
...
void ExportLy::doSlurStart(Chord* chord, bool nextisrest)
{
    ...
    for (const Spanner* sp = chord->spannerFor(); sp; sp = sp->next()) {
        ...
        if (i >= 0) {
            ...
            slurre[i] = 0;
            started[i] = false;
            if (s->slurDirection() == MScore::UP) {
                out << "^";
            }
            if (s->slurDirection() == MScore::DOWN) {
                out << "_";
            }
            if (slurcount == 2) {
...
void ExportLy::findFingerAndStringno(Note* note, int& fingix, int& stringix, QString (& fingarray)[5],
...
{
    if (strgix > 0) { // there should be only one stringinstruction, so this is possibly redundant.
...
void ExportLy::findGraceNotes(Note* note, bool& chordstart, int streng)
{
    ...
    case NOTE_NORMAL: {
        if (graceswitch == true) {
            ...
            gracebeam = false;
            if (gracecount > 1) {
                out << " ] ";             // single graces are not beamed
            }
...
void ExportLy::setOctave(int& purepitch, int& pitchidx, int (& pitchlist)[12])
{
    ...
    while (oktreit > 0)
    {
        if ((oktavdiff < -6) or ((prevnote == "b") and (oktavdiff < -5))) { // up
            out << "'";
            oktavdiff = oktavdiff + 12;
        } else if ((oktavdiff > 6)  or ((prevnote == "f") and (oktavdiff > 5))) {// down
            out << ",";
            oktavdiff = oktavdiff - 12;
        }
        oktreit = oktreit - 12;
...
void ExportLy::writeChord(Chord* c, bool nextisrest)
{
    ...
    for (QList<Note*>::iterator notesinchord = nl.begin();;) {
        ...
        if (streng > 0) {
            writeStringInstruction(streng,stringno);
        }
    ...
    {
        if (pitchlist[ix] < prevpitch) {
            prevpitch = pitchlist[ix];
        }
        ix++;
    }
...
int ExportLy::getLen(int l, int* dots)
{
    ...
    // so we set len equal to nominal value
        ...
        qDebug("measure: %d, unsupported len %d (%d,%d)\n", measurenumber, l, l / MScore::division,
               l % MScore::division);
    }
    return len;
}
        ...
        while (utgang == false)
        {
            ...
            case startending: {
                if (firstalt == false) {
                    ...
                    if (wholemeasurerest > 0) {
                        writeMeasuRestNum();                // should not happen?
                    }
                    out << "{ ";
                    indent();
                    firstalt = false;
                ...
                foreach (const Lyrics* lix, * lyrlist) {
                    if (lix) {
                        ...
                        if (verse > thisLyrics->numberofverses) {
                            ...
                            if (verse > 0) {
                                int segdiff
                                    = (thisLyrics->lyrdat.segmentnumber[verse - 1]
                                       - thisLyrics->lyrdat.segmentnumber[verse]);
                                if (segdiff > 0) {
...
// out << "\\time " << z1<< "/" << timedenom << " \n";
    ...
    for (Segment* s = m->first(); s; s = s->next()) {
        ...
        case Element::TIMESIG:
        {
            ...
            if (timedenom == 8) {
                nombarlen = nombarlen / 2;
            }
            if (timedenom == 2) {
                nombarlen = 2 * nombarlen;
            }
...
void ExportLy::writePageFormat()
{
    ...
    if (pf->size().width() > pf->size().height()) {
        os << " 'landscape";
    }
