...
void FotoLasso::startEdit(EditData& ed)
{
    Lasso::startEdit(ed);
    QRectF view
        = ((ScoreView*)ed.view)->toLogical(QRect(0.0, 0.0, ed.view->geometry().width(), ed.view->geometry().height()));
...
void ScoreView::fotoModeCopy(bool includeLink)
{
    ...
    if (includeLink) {
        ...
        buffer.close();
        QString html = "<a href=\"" + url.toString() + "\"><img src=\"data:image/png," + imageData.toPercentEncoding()
                       + "\" /></a>";
