...
void InspectorBase::checkDifferentValues(const InspectorItem& ii)
{
    ...
    if (inspector->el()->size() > 1) {
        ...
        ii.w->setStyleSheet(valuesAreDifferent ? QString(
                                "* { color: %1; } QToolTip { color: palette(tooltiptext); }").arg(c.name()) : " ");
    }
...
#endif
    InspectorEventObserver::instance()->event(
        reset ? InspectorEventObserver::PropertyReset : InspectorEventObserver::PropertyChange, ii, iElement);
...
void InspectorBase::mapSignals(const std::vector<InspectorItem>& il, const std::vector<InspectorPanel>& pl)
{
    ...
    for (const InspectorItem& ii : iList) {
        ...
        if (qobject_cast<QDoubleSpinBox*>(w)) {
            connect(qobject_cast<QDoubleSpinBox*>(w), QOverload<double>::of(&QDoubleSpinBox::valueChanged), [=] {
                    valueChanged(i);
                });
        } else if (qobject_cast<QSpinBox*>(w)) {
            connect(qobject_cast<QSpinBox*>(w), QOverload<int>::of(&QSpinBox::valueChanged), [=] { valueChanged(i); });
        } else if (qobject_cast<QFontComboBox*>(w)) {
            connect(qobject_cast<QFontComboBox*>(w), QOverload<const QFont&>::of(
                        &QFontComboBox::currentFontChanged), [=] { valueChanged(i); });
        } else if (qobject_cast<QComboBox*>(w)) {
            connect(qobject_cast<QComboBox*>(w), QOverload<int>::of(&QComboBox::currentIndexChanged), [=] {
                    valueChanged(i);
                });
        } else if (qobject_cast<QCheckBox*>(w)) {
            connect(qobject_cast<QCheckBox*>(w), QOverload<bool>::of(&QCheckBox::toggled), [=] { valueChanged(i); });
        } else if (qobject_cast<Awl::ColorLabel*>(w)) {
            connect(qobject_cast<Awl::ColorLabel*>(w), QOverload<QColor>::of(&Awl::ColorLabel::colorChanged), [=] {
                    valueChanged(i);
                });
        } else if (qobject_cast<QRadioButton*>(w)) {
            connect(qobject_cast<QRadioButton*>(w), QOverload<bool>::of(&QRadioButton::toggled), [=] {
                    valueChanged(i);
                });
        } else if (qobject_cast<QPushButton*>(w)) {
            connect(qobject_cast<QPushButton*>(w), QOverload<bool>::of(&QPushButton::toggled),
                    [=] { valueChanged(i); });
        } else if (qobject_cast<QToolButton*>(w)) {
            connect(qobject_cast<QToolButton*>(w), QOverload<bool>::of(&QToolButton::toggled),
                    [=] { valueChanged(i); });
        } else if (qobject_cast<QLineEdit*>(w)) {
            connect(qobject_cast<QLineEdit*>(w), QOverload<const QString&>::of(&QLineEdit::textChanged), [=] {
                    valueChanged(i);
                });
        } else if (qobject_cast<Ms::AlignSelect*>(w)) {
            connect(qobject_cast<Ms::AlignSelect*>(w), QOverload<Align>::of(&Ms::AlignSelect::alignChanged), [=] {
                    valueChanged(i);
                });
        } else if (qobject_cast<Ms::FontStyleSelect*>(w)) {
            connect(qobject_cast<Ms::FontStyleSelect*>(w),
                    QOverload<FontStyle>::of(&Ms::FontStyleSelect::fontStyleChanged), [=] { valueChanged(i); });
        } else if (qobject_cast<Ms::OffsetSelect*>(w)) {
            connect(qobject_cast<Ms::OffsetSelect*>(w), QOverload<const QPointF&>::of(
                        &Ms::OffsetSelect::offsetChanged), [=] { valueChanged(i); });
        } else if (qobject_cast<Ms::ScaleSelect*>(w)) {
            connect(qobject_cast<Ms::ScaleSelect*>(w), QOverload<const QSizeF&>::of(
                        &Ms::ScaleSelect::scaleChanged), [=] {
                    valueChanged(i);
                });
        } else if (qobject_cast<Ms::SizeSelect*>(w)) {
            connect(qobject_cast<Ms::SizeSelect*>(w), QOverload<const QVariant&>::of(
                        &Ms::SizeSelect::valueChanged), [=] {
                    valueChanged(i);
                });
        } else {
            qFatal("not supported widget %s", w->metaObject()->className());
...
void InspectorBase::resetToStyle()
{
    ...
    for (Element* e : *inspector->el()) {       // TODO: ??
        ...
        // Preserve <sym> tags
        text->undoChangeProperty(Pid::TEXT,
                                 text->plainText().toHtmlEscaped().replace("&lt;sym&gt;",
                                                                           "<sym>").replace("&lt;/sym&gt;","</sym>"));
    }
    score->endCmd();
}
