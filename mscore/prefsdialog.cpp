...
void PreferenceDialog::updateValues(bool useDefaultValues)
{
    ...
    jackDriver->setChecked(preferences.getBool(PREF_IO_JACK_USEJACKAUDIO)
                           || preferences.getBool(PREF_IO_JACK_USEJACKMIDI));
    ...
    case SessionStart::EMPTY: {
        emptySession->setChecked(true);
    }
    break;
    case SessionStart::LAST: {
        lastSession->setChecked(true);
    }
    break;
    ...
    if (preferences.getBool(PREF_EXPORT_MUSICXML_EXPORTLAYOUT)) {
        ...
        case MusicxmlExportBreaks::ALL: {
            exportAllBreaks->setChecked(true);
        }
        break;
        case MusicxmlExportBreaks::MANUAL: {
            exportManualBreaks->setChecked(true);
        }
        break;
        case MusicxmlExportBreaks::NO: {
            exportNoBreaks->setChecked(true);
        }
        break;
        }
    }
    ...
    if (portAudioIsUsed) {
        ...
        if (audio) {
            ...
            connect(portaudioApi, QOverload<int>::of(
                        &QComboBox::activated), this, &PreferenceDialog::portaudioApiActivated);
#ifdef USE_PORTMIDI
...
void PreferenceDialog::saveShortcutListClicked()
{
    QString saveFileName
        = QFileDialog::getSaveFileName(this, tr("Save Shortcuts"), preferences.getString(
                                           PREF_APP_PATHS_MYSHORTCUTS) + "/shortcuts.xml", tr(
                                           "MuseScore Shortcuts File") + " (*.xml)", 0, preferences.getBool(
...
void PreferenceDialog::loadShortcutListClicked()
{
    QString loadFileName
        = QFileDialog::getOpenFileName(this, tr("Load Shortcuts"), preferences.getString(PREF_APP_PATHS_MYSHORTCUTS), tr(
                                           "MuseScore Shortcuts File") + " (*.xml)", 0, preferences.getBool(
                                           PREF_UI_APP_USENATIVEDIALOGS) ? QFileDialog::Options() : QFileDialog::DontUseNativeDialog);
...
void PreferenceDialog::filterShortcutsTextChanged(const QString& query)
{
    ...
    for (int i = 0; i < shortcutList->topLevelItemCount(); i++) {
        ...
        item->setHidden(!(item->text(0).contains(query,
                                                 Qt::CaseInsensitive)
...
void PreferenceDialog::apply()
{
    ...
    bool jackParametersChanged = (preferences.getBool(PREF_IO_JACK_USEJACKAUDIO) != wasJackAudio
                                  ...
                                  || preferences.getBool(PREF_IO_JACK_REMEMBERLASTCONNECTIONS)
                                  != rememberLastMidiConnections->isChecked()
                                  || preferences.getBool(PREF_IO_JACK_TIMEBASEMASTER)
                                  != becomeTimebaseMaster->isChecked())
                                 && (wasJack && nowJack);
    ...
    break;
    case 3: {
        ticks = MScore::division / 8;
    }
    break;
    case 4: {
        ticks = MScore::division / 16;
    }
    break;
    case 5: {
        ticks = MScore::division / 32;
    }
    break;
    case 6: {
        ticks = MScore::division / 64;
    }
    break;
    case 7: {
        ticks = MScore::division / 128;
    }
    break;
    case 8: {
        ticks = MScore::division / 256;
    }
    break;
    default: {
...
void PreferenceDialog::exclusiveAudioDriver(bool on)
{
    if (on) {
        ...
        bool alsaChecked = alsaDriver->isVisible()
                           && ((QObject::sender() != alsaDriver && alsaDriver->isChecked())
                               || QObject::sender() == alsaDriver);
