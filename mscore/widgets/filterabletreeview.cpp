    ...
    T(parent)
{
    ...
    FilterableTreeViewTemplate<T>::connect(this, &FilterableTreeViewTemplate<T>::clicked, this,
                                           &FilterableTreeViewTemplate<T>::toggleExpandedForUnselectable);
    // expand current branch when activated (usually when double-clicked, or when selected and Enter key is pressed)
    FilterableTreeViewTemplate<T>::connect(this, &FilterableTreeViewTemplate<T>::activated, this,
                                           &FilterableTreeViewTemplate<T>::toggleExpanded);
}
...
// selectFirst();
    const auto lambdaNodeIsVisibleSelectable = [this](QModelIndex idx) -> bool {
                                                   return idx.flags() & Qt::ItemIsSelectable
                                                          && !FilterableTreeViewTemplate<T>::isIndexHidden(idx);
                                               };
