...
void ScoreView::mousePressEventNormal(QMouseEvent* ev)
{
    ...
    if (e) {
        ...
        } else if (e->isTimeSig() && !toTimeSig(e)->isLocal() && (keyState != Qt::ControlModifier)
                   && st == SelectType::SINGLE) {
...
void ScoreView::mouseDoubleClickEvent(QMouseEvent* mouseEvent)
{
    ...
    if (!clickedElement->isEditable()) {
        ...
        } else if (clickedElement->isText()
                   && (toText(clickedElement)->tid() == Tid::FOOTER || toText(clickedElement)->tid() == Tid::HEADER)) {                          // double-click an instrument name to open the edit staff/part properties menu
...
void ScoreView::keyPressEvent(QKeyEvent* ev)
{
    ...
    if (MScore::debugMode) {
        qDebug("keyPressEvent key 0x%02x(%c) mod 0x%04x <%s> nativeKey 0x%02x scancode %d",
               editData.key, editData.key, int(editData.modifiers), qPrintable(editData.s),
               ev->nativeVirtualKey(), ev->nativeScanCode());
    }
...
static const char* stateName(ViewState s)
{
    ...
    case ViewState::FOTO_DRAG_EDIT: {
        return "FOTO_DRAG_EDIT";
    }
    case ViewState::FOTO_DRAG_OBJECT: {
        return "FOTO_DRAG_OBJECT";
    }
...
// }
    ...
    case ViewState::FOTO: {
        setCursor(QCursor(Qt::ArrowCursor));
        if (state != ViewState::FOTO_DRAG && state != ViewState::FOTO_DRAG_EDIT
            && state != ViewState::FOTO_DRAG_OBJECT) {
            startFotomode();
        }
