...
int Model::read_instr()
{
    ...
    Addsynth* A;
      ...
      #undef ERROR
    enum {
        CONT, DONE, ERROR, COMM, ARGS, MORE, NO_INSTR, IN_INSTR,
        ...
        if (!strcmp(p, "/instr/new")) {
            ...
            if (D || G) {
                ...
                if (_ndivis == NDIVIS) {
                    ...
                    stat = BAD_STR1;
                } else if ((k < 0) || (k > _nkeybd)) {
                    stat = BAD_KEYBD;
                } else if ((s < 1) || (s > NASECT)) {
                    stat = BAD_ASECT;
                } else {
                    D = _divis + _ndivis++;
                    strcpy(D->_label, t1);
            ...
            if (!D && G) {
                stat = BAD_SCOPE;
            } else if (sscanf(q, "%c%d%s%n", &c, &d, t1, &n) != 3) {
                stat = ARGS;
            } else {
                q += n;
            ...
            if (D) {
                ...
                if (sscanf(q, "%d%s%s%n", &d, t1, t2, &n) != 3) {
                    stat = ARGS;
                } else {
                    q += n;
                    if (G->_nifelm == Group::NIFELM) {
                        stat = BAD_IFACE;
                    } else if ((d < 1) || (d > _ndivis)) {
                        stat = BAD_DIVIS;
                    } else if (strlen(t1) > 7) {
                        stat = BAD_STR1;
                    } else if (strlen(t2) > 31) {
                        stat = BAD_STR2;
                    } else {
                        d--;
            ...
            if (!D || G) {
                ...
                D->_flags |= Divis::HAS_SWELL;
            }
        } else if (!strcmp(p, "/stop")) {
            if (D || !G) {
                stat = BAD_SCOPE;
            } else if (sscanf(q, "%d%d%d%n", &k, &d, &r, &n) != 3) {
                stat = ARGS;
            } else {
                q += n;
                if (G->_nifelm == Group::NIFELM) {
                    ...
                    stat = BAD_DIVIS;
                } else if ((r < 1) || (r > _divis [d - 1]._nrank)) {
                    stat = BAD_RANK;
                } else {
                    k--;
                    d--;
            ...
            if (D || !G) {
                stat = BAD_SCOPE;
            } else if (sscanf(q, "%d%d%s%s%n", &k, &d, t1, t2, &n) != 4) {
                stat = ARGS;
            } else {
                q += n;
                if (G->_nifelm == Group::NIFELM) {
                    stat = BAD_IFACE;
                } else if ((k < 1) || (k > _nkeybd)) {
                    stat = BAD_KEYBD;
                } else if ((d < 1) || (d > _ndivis)) {
                    stat = BAD_DIVIS;
                } else if (strlen(t1) > 7) {
...
int Model::write_instr()
{
    ...
    fprintf(F, "\n# Keyboards\n#\n");
    for (k = 0; k < _nkeybd; k++) {
        if (_keybd [k]._flags & Keybd::IS_PEDAL) {
            fprintf(F, "/pedal/new    %s\n", _keybd [k]._label);
        } else {
            fprintf(F, "/manual/new   %s\n", _keybd [k]._label);
        }
    }
