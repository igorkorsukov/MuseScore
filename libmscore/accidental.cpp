...
void Accidental::layout()
{
    ...
    if (_bracket != AccidentalBracket::NONE) {
        SymId id = _bracket
                   == AccidentalBracket::PARENTHESIS ? SymId::accidentalParensLeft : SymId::accidentalBracketLeft;
        SymElement e(id, 0.0);
        el.append(e);
        r |= symBbox(id);
    }
    ...
    if (_bracket != AccidentalBracket::NONE) {
        SymId id = _bracket
                   == AccidentalBracket::PARENTHESIS ? SymId::accidentalParensRight : SymId::accidentalBracketRight;
        x = r.x() + r.width();
        SymElement e1(id, x);
        el.append(e1);
...
QVariant Accidental::propertyDefault(Pid propertyId) const
{
    switch (propertyId) {
    case Pid::ACCIDENTAL_TYPE: {
        return int(AccidentalType::NONE);
    }
    case Pid::SMALL: {
        return false;
    }
    case Pid::ACCIDENTAL_BRACKET: {
        return int(AccidentalBracket::NONE);
    }
    case Pid::ROLE: {
        return int(AccidentalRole::AUTO);
    }
    default:
