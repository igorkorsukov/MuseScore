...
static void match_profiles()
{
    ...
    for (s = 0; s <= segtotal; ++s) {
        if (scoring_mode == 0) {
            ...
            if (verbosity == 3) {
                qDebug("For segment %d: average_dur = %6.3f; input_sumsq = %6.3f", s, segment[s].average_dur,
                       input_sumsq);
            }
        }
        best_key = 0;
...
static void best_key_analysis()
{
    ...
    if (verbosity >= 2) {
        qDebug("Segment 0: key choice %d; total score %6.3f; segment score %6.3f", final[0], keyscore()[final[0]][0],
               keyscore()[final[0]][0]);
        qDebug("Segment 1: key choice %d; total score %6.3f; segment score %6.3f", final[1],
               analysis[final[0]][final[1]][1], analysis[final[0]][final[1]][1] - keyscore()[final[0]][0]);
        for (s = 2; s <= segtotal; s++) {
            qDebug("Segment %d: key choice %d; total score %6.3f; segment score %6.3f", s, final[s],
                   analysis[final[s - 1]][final[s]][s],
