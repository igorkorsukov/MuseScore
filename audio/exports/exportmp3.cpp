...
bool MP3Exporter::loadLibrary(AskUser askuser)
{
    ...
    if (!validLibraryLoaded() && askuser != MP3Exporter::AskUser::NO) {
        ...
        int ret = QMessageBox::question(0, qApp->translate("MP3Exporter", "Save as MP3"),
                                        qApp->translate("MP3Exporter",
                                                        "MuseScore does not export MP3 files directly, but instead uses "
                                                        ...
                                                        "Would you like to locate %2 now?").arg(
                                            getLibraryName()).arg(getLibraryName()),
...
bool MP3Exporter::initLibrary(QString libpath)
{
    ...
    if (!lame_init
        ...
        if (beVersion) {
            ...
            mBladeVersion = QString(
                "You are linking to lame_enc.dll v%d.%d. This version is not compatible with MuseScore %d.\nPlease download the latest version of the LAME MP3 library.")
...
{
    return QString(
        "Only libmp3lame.so.0 (libmp3lame.so.0);;Primary Shared Object files (*.so);;Extended Libraries (*.so*);;All Files (*)");
}
