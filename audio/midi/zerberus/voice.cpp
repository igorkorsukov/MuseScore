...
void Voice::start(Channel* c, int key, int v, const Zone* zone, double durSinceNoteOn)
{
    ...
    envelopes[V1Envelopes::DELAY].setTime(z->ampegDelay + (z->ampegVel2Delay * velPercent) + z->delay,
                                          _zerberus->sampleRate());
    ...
    if (trigger == Trigger::RELEASE || trigger == Trigger::CC) {
        ...
        double sustainDur = scaledSampleDur
                            - (z->ampegDelay + z->ampegAttack + z->ampegHold + z->ampegDecay + z->ampegRelease
                               + z->delay);
    ...
    envelopes[V1Envelopes::SUSTAIN].setConstant(qBound(0.0f, z->ampegSustain + (z->ampegVel2Sustain * velPercent),
                                                       1.0f));

    envelopes[V1Envelopes::RELEASE].setTable(Envelope::egPow);
    envelopes[V1Envelopes::RELEASE].setVariable();
    envelopes[V1Envelopes::RELEASE].setTime(z->ampegRelease + (z->ampegVel2Release * velPercent),
                                            _zerberus->sampleRate());
...
// printf("end of sample\n");
            ...
            float interpValR = filter.interpolate(phase.fract(),
                                                  getData(idx - 1), getData(idx + 1), getData(idx + 3),
                                                  getData(idx + 5));
...
void Voice::updateLoop()
{
    ...
    bool shallLoop = loopMode() == LoopMode::CONTINUOUS
                     || (loopMode() == LoopMode::SUSTAIN && (_state < VoiceState::STOP));
