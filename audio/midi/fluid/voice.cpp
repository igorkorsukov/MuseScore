...
bool Voice::generateDataForDSPChain(unsigned framesBufCount)
{
    ...
    if (modenv_section < 2 && modenv_data[FLUID_VOICE_ENVDELAY].count < 100
        && modenv_data[FLUID_VOICE_ENVATTACK].count < 100) {
        modenv_section = 2;
        modenv_val = 1.0f;
    }
...
// printf("!sample\n");
    ...
    if (n <= _cachedFrames) {
        ...
        const unsigned requiredNumberOfFramesToGenerateEnvelope = FLUID_VOICE_ENVLAST
                                                                  * volenv_data[FLUID_VOICE_ENVDELAY].count;
        const unsigned framesToGenerateData
            = std::max(leftBufferFramesToFill, requiredNumberOfFramesToGenerateEnvelope);
...
void Voice::update_param(int _gen)
{
    ...
    case GEN_ATTENUATION: {
        attenuation = gen[GEN_ATTENUATION].val * ALT_ATTENUATION_SCALE + gen[GEN_ATTENUATION].mod
                      + gen[GEN_ATTENUATION].nrpn;
...
void Voice::check_sample_sanity()
{
     ...
     */
    if (((SAMPLEMODE() == FLUID_LOOP_UNTIL_RELEASE) && (volenv_section < FLUID_VOICE_ENVRELEASE))
        || (SAMPLEMODE() == FLUID_LOOP_DURING_RELEASE)) {
