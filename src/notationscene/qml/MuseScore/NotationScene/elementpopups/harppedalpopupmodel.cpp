...
void HarpPedalPopupModel::load()
{
    mu::engraving::HarpPedalDiagram* diagram = m_item
                                               && m_item->isHarpPedalDiagram() ? toHarpPedalDiagram(
        m_item) : nullptr;
    if (!diagram) {
        return;
    }
...
std::array<mu::engraving::PedalPosition,
           mu::engraving::HARP_STRING_NO> HarpPedalPopupModel::getPopupPedalState()
{
...
void HarpPedalPopupModel::setIsDiagram(bool isDiagram)
{
    ...
    changeItemProperty(mu::engraving::Pid::HARP_IS_DIAGRAM, m_isDiagram,
