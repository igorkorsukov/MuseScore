   ...
   // MYLOG() << "item: " << item->accessibleName();
    ...
    item->accessibleParentChanged().onNotify(this, [this, item]() {
        const Item& it = findItem(item);
                LOGI() << "accessibleParentChanged: " << item->accessibleName() << ", parent: " << item->accessibleParent()->accessibleName();
        if (!it.isValid()) {
            return;
        }
...
void AccessibilityController::focused(IAccessibility* aitem)
{
    ...
    if (item.item->accessibleName() == "Publish") {
        const Item& root = findItem(rootItem());
        debug_dumpTree(root.iface);
    }
...
//#ifdef ACCESSIBILITY_LOGGING_ENABLED
    ...
    LOGI() << "object: " << obj->item()->accessibleName() << ", event: " << int(ev->type());
//#endif
