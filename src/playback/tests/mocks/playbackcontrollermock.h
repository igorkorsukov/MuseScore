...
public:
    ...
    MOCK_METHOD((muse::async::Channel<muse::audio::secs_t, muse::midi::tick_t>),
                currentPlaybackPositionChanged, (), (const, override));
    ...
    MOCK_METHOD((muse::async::Channel<muse::audio::aux_channel_idx_t, std::string>),
                auxChannelNameChanged, (), (const, override));

    MOCK_METHOD(muse::async::Promise<muse::audio::SoundPresetList>, availableSoundPresets,
    ...
    MOCK_METHOD(const SoloMuteState&, trackSoloMuteState, (const engraving::InstrumentTrackId&),
                (const, override));
    MOCK_METHOD(void, setTrackSoloMuteState,
                (const engraving::InstrumentTrackId&, const SoloMuteState&),
                (override));

    MOCK_METHOD(void, playElements,
                (const std::vector<const notation::EngravingItem*>&, const PlayParams&, bool),
                (override));
    MOCK_METHOD(void, playNotes,
                (const notation::NoteValList&, notation::staff_idx_t, const notation::Segment*,
                 const PlayParams&),
                (override));
    MOCK_METHOD(void, playMetronome, (int), (override));

    MOCK_METHOD(void, triggerControllers,
                (const muse::mpe::ControllerChangeEventList&, notation::staff_idx_t, int),
                (override));
    ...
    MOCK_METHOD(muse::async::Channel<muse::actions::ActionCode>, actionCheckedChanged, (),
                (const, override));
    ...
    MOCK_METHOD((const std::map<muse::audio::TrackId, muse::audio::AudioResourceMeta>&),
                onlineSounds, (), (const, override));
