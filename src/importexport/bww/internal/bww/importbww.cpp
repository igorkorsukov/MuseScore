...
public:
    ...
    void header(const QString title, const QString type, const QString composer,
                const QString footer, const unsigned int temp);
    void note(const QString pitch, const QVector<Bww::BeamType> beamList, const QString type,
              const int dots, bool tieStart = false, bool tieStop = false,
              StartStop triplet = StartStop::ST_NONE, bool grace = false);
...
void MsScWriter::beginMeasure(const Bww::MeasureBeginFlags mbf)
{
    ...
    if (measureNumber == 1) {
        // clef
        mu::engraving::Segment* s = currentMeasure->getSegment(
            mu::engraving::SegmentType::HeaderClef, tick);
                      ...
                      bool grace)
{
    ...
    mu::engraving::BeamMode bm
        = (beamList.at(0)
           == Bww::BeamType::BM_BEGIN) ? mu::engraving::BeamMode::BEGIN : mu::engraving::BeamMode::
          AUTO;
    ...
    if (!grace) {
        mu::engraving::Segment* s = currentMeasure->getSegment(
            mu::engraving::SegmentType::ChordRest, tick);
        s->add(cr);
