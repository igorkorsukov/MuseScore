...
static void translate_dashPattern(QVector<qreal> pattern, const qreal& width,
...
public:
    SvgPaintEnginePrivate()
    {
        ...
        attributes.description = QString("Generated by MuseScore Studio %1").arg(
...
void SvgGenerator::setOutputDevice(QIODevice* outputDevice)
{
    ...
    if (d->engine->isActive()) {
        LOGW(
            "SvgGenerator::setOutputDevice(), cannot set output device while SVG is being generated");
        return;
    }
    d->owns_iodevice = false;
...
void SvgPaintEngine::writeImage(const QRectF& r, const QByteArray& imageData,
...
void SvgPaintEngine::defineClipPath(const QPainterPath& clipPath)
{
    ...
    if (_replaceClipPathWithMask) {
        stream() << SVG_DEFS_BEGIN << SVG_SPACE << SVG_MASK_BEGIN << SVG_ID << _curClipPathId <<
        SVG_QUOTE << SVG_GT << SVG_SPACE;
    } else {
        stream() << SVG_DEFS_BEGIN << SVG_SPACE << SVG_CLIP_PATH_BEGIN << SVG_ID <<
        _curClipPathId << SVG_QUOTE << SVG_GT << SVG_SPACE;
    }
    ...
    if (_replaceClipPathWithMask) {
        stream() << SVG_FILL_RULE << SVG_FILL << "white\"" << SVG_ELEMENT_END << SVG_MASK_END <<
        SVG_DEFS_END << Qt::endl;
    } else {
        stream() << SVG_CLIP_RULE << SVG_ELEMENT_END << SVG_CLIP_PATH_END << SVG_DEFS_END <<
        Qt::endl;
    }
}
