...
MScoreTextToMusicXml::MScoreTextToMusicXml(const String& tag, const String& attr,
                                           const CharFormat& defFmt, const String& mtf)
    ...
    while (fragmentNr != in.end()) {
        ...
        if ((currentDest != &right && destSize + fragSize <= currentMaxSize)
            ...
            TextFragment rightPart
                = fragment.split(currentMaxSize - plainTextPlusSymbolsListSize(*currentDest));
