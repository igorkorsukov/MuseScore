...
public:
    ...
    int getNextFreeMidiMapping(std::set<int>& occupiedMidiChannels,
                               unsigned int& searchMidiMappingFrom, int p = -1, int ch = -1);
...
private:
    ...
    void doUpdateMidiMapping(int& maxport, std::set<int>& occupiedMidiChannels,
                             unsigned int& searchMidiMappingFrom, Part* part, InstrChannel* channel,
                             bool useDrumset);
