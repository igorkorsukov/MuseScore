               ...
               ElementFlag::MOVABLE | ElementFlag::ON_STAFF)
{
    ...
    m_pedalState
        = std::array<PedalPosition, HARP_STRING_NO> { PedalPosition::NATURAL,
...
String HarpPedalDiagram::createDiagramText()
{
    ...
    if (m_isDiagram) {
        ...
        constexpr std::array<PedalPosition, HARP_STRING_NO> initState
            = { PedalPosition::UNSET, PedalPosition::UNSET, PedalPosition::UNSET,
...
{
    String rez
        = score() ? score()->getTextStyleUserName(textStyleType()).translated() : TConv::
          translatedUserName(
              textStyleType());
    String s;
