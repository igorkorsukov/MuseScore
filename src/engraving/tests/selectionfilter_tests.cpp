...
{
    Score* score = ScoreRW::readScore(SELECTIONFILTER_DATA_DIR + String(
                                          u"selectionfilter%1.mscx").arg(idx));
    EXPECT_TRUE(score);
    score->doLayout();
    ...
    EXPECT_TRUE(ScoreComp::saveCompareMimeData(score->selection().mimeData(),
                                               String("selectionfilter%1.xml").arg(idx),
                                               SELECTIONFILTER_DATA_DIR
...
void Engraving_SelectionFilterTests::testFilterSpanner(int idx,
                                                       const SelectionFilterTypesVariant& type)
{
    Score* score = ScoreRW::readScore(SELECTIONFILTER_DATA_DIR + String(
                                          "selectionfilter%1.mscx").arg(idx));
    EXPECT_TRUE(score);
    score->doLayout();
                                                                            ...
                                                                            MeasureBase* auxMeasure), const String& reference)
{
    ...
    MasterScore* score
        = ScoreRW::readScore(SELECTIONFILTER_DATA_DIR + String(
                                 u"selectionfilter_notesinchords.mscx"));
    EXPECT_TRUE(score);
...
TEST_F(Engraving_SelectionFilterTests, notesInChordsDeleteRange)
{
    ...
    testNotesInChordsAction(deleteAction,
                            SELECTIONFILTER_DATA_DIR
                            + String(u"selectionfilter_notesinchords_deleterange-ref.mscx"));
}
...
TEST_F(Engraving_SelectionFilterTests, notesInChordsCopyPaste)
{
    ...
    testNotesInChordsAction(copyPasteAction,
                            SELECTIONFILTER_DATA_DIR
                            + String(u"selectionfilter_notesinchords_copypaste-ref.mscx"));
}
...
TEST_F(Engraving_SelectionFilterTests, gracesAndSlurs)
{
    ...
    MasterScore* score
        = ScoreRW::readScore(SELECTIONFILTER_DATA_DIR
                             + String(u"selectionfilter_gracesandslurs.mscx"));
    EXPECT_TRUE(score);
    ...
    EXPECT_TRUE(ScoreComp::saveCompareScore(score,
                                            String(u"selectionfilter_gracesandslurs.mscx.mscx"),
                                            SELECTIONFILTER_DATA_DIR
                                            + String("selectionfilter_gracesandslurs-ref.mscx")));
}
