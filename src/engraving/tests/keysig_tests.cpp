...
TEST_F(Engraving_KeySigTests, preferSharpFlat)
{
    ...
    Transpose::transpositionChanged(score1, part1, part1->instrument(Fraction(0,
                                                                              1))->transpose(), Fraction(
                                        0,
                                        1), Fraction(16, 4));
    score1->update();
    score1->doLayout();
    ...
    Transpose::transpose(score2, TransposeMode::BY_INTERVAL, TransposeDirection::UP, Key::C, 1,
                         true, true, true);
    score2->endCmd();
...
TEST_F(Engraving_KeySigTests, keysigMode)
{
    ...
    score->update();
    score->doLayout();
    EXPECT_TRUE(ScoreComp::saveCompareScore(score, u"keysig03.mscx",
                                            KEYSIG_DATA_DIR + u"keysig03-ref.mscx"));
    delete score;
}
