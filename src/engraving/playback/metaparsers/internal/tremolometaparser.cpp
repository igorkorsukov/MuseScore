                                      ...
                                      mpe::ArticulationMap& result)
{
    ...
    articulationMeta.overallDuration = timestampFromTicks(ctx.score,
...
void TremoloTwoMetaParser::doParse(const EngravingItem* item, const RenderingContext& ctx,
                                   mpe::ArticulationMap& result)
{
    ...
    if (tremolo->chord1() && tremolo->chord2()) {
        overallDurationTicks = tremolo->chord1()->actualTicks().ticks()
                               + tremolo->chord2()->actualTicks().ticks();
    }
