    ...
    for (size_t staffIdx = 0; staffIdx < nstaves; ++staffIdx) {
        ...
        for (size_t i = 0; i < columns; ++i) {
            for (auto bi : s->brackets()) {
                ...
                Bracket* b = createBracket(system, ctx, bi, i, static_cast<int>(staffIdx), bl,
                                           system->firstMeasure());
                if (b != nullptr) {
...
Bracket* SystemHeaderLayout::createBracket(System* system, LayoutContext& ctx, BracketItem* bi,
                                           size_t column, staff_idx_t staffIdx,
    ...
    if (span > 1
        ...
        for (size_t k = 0; k < bl.size(); ++k) {
            if (bl[k]->track() == track && bl[k]->column() == column
                && bl[k]->bracketType() == bi->bracketType()
    ...
    for (staff_idx_t staffIdx = 0; staffIdx < nstaves; ++staffIdx) {
        ...
        for (auto bi : staff->brackets()) {
            ...
            if (span > 1
                || (bi->bracketSpan() == span)
                || (span == 1
                    && ctx.conf().styleB(Sid::alwaysShowBracketsWhenEmptyStavesAreHidden))) {
                Bracket* dummyBr = Factory::createBracket(
                    ctx.mutDom().dummyParent(), /*isAccessibleEnabled=*/ false);
...
bool SystemHeaderLayout::stackLabelsVertically(System* system)
{
    ...
    InstrumentNamesAlign align
        = style.styleV(longName ? Sid::instrumentNamesAlignLong : Sid::instrumentNamesAlignShort).
          value<InstrumentNamesAlign>();
    return align == InstrumentNamesAlign::CENTER_CENTER
           || (align == InstrumentNamesAlign::RIGHT_RIGHT
               && style.styleB(Sid::instrumentNamesStackVertically));
}
...
void SystemHeaderLayout::computeInstrumentNameOffset(System* system, LayoutContext& ctx)
{
    ...
    if (!followStaffSize) {
        textSizeScaling *= DefaultStyle::defaultStyle().value(Sid::spatium).toDouble()
                           / ctx.conf().styleD(Sid::spatium);
    }
...
void SystemHeaderLayout::computeGroupBracketsWidths(System* system, LayoutContext& ctx)
{
    ...
    std::sort(groupBrackets.begin(), groupBrackets.end(), [](Bracket* b1, Bracket* b2) {
        return b1->column() < b2->column();
    });

    bool hangIntoMargin = ctx.conf().styleB(Sid::groupBracketHangTextIntoMargin)
                          && ctx.conf().styleV(Sid::groupBracketTextOrientation).value<Orientation>()
...
void SystemHeaderLayout::computeStackedBracketsWidth(Bracket* first,
                                                     const std::vector<Bracket*>& allGroupBracketsOrderedByColumn,
...
void SystemHeaderLayout::computeInstrumentNamesWidth(System* system, LayoutContext& ctx)
{
    ...
    for (staff_idx_t staffIdx = 0; staffIdx < system->staves().size(); ++staffIdx) {
        ...
        if (InstrumentName* groupName = sysStaff->groupName) {
            ...
            for (staff_idx_t groupIdx = groupName->staffIdx();
                 groupIdx < groupName->ldata()->endIdxOfGroup(); ++groupIdx) {
                partsWithGroupNames.insert(ctx.dom().staff(groupIdx)->part());
            }
            ldata->setSecondColumnWidth(std::max(ldata->secondColumnWidth(),
                                                 nameWidthIncludingGroupBrackets(groupName,
                                                                                 system)));
            ldata->setTotalNamesWidth(std::max(ldata->totalNamesWidth(),
                                               nameWidthIncludingGroupBrackets(groupName, system)));
        }
        ...
        if (InstrumentName* name = sysStaff->individualStaffName) {
            ...
            ldata->setTotalNamesWidth(std::max(ldata->totalNamesWidth(),
                                               nameWidthIncludingGroupBrackets(name, system)));
        }
    }
    ...
    for (staff_idx_t staffIdx = 0; staffIdx < system->staves().size(); ++staffIdx) {
        ...
        ldata->setTotalNamesWidth(std::max(ldata->totalNamesWidth(),
                                           nameWidthIncludingGroupBrackets(instrName, system)));
        ...
        if (partsWithGroupNames.count(part)) {
            ...
            ldata->setSecondColumnWidth(std::max(ldata->secondColumnWidth(),
                                                 nameWidthIncludingGroupBrackets(instrName,
                                                                                 system)));
        }
    }
    ...
    auto sumWidth = [&](InstrumentName* outerName, InstrumentName* innerName) {
        ...
        if (staffNameAlign == AlignH::LEFT || staffNameAlign == AlignH::JUSTIFY) {
            return ldata->firstColumnWidth()
                   + nameWidthIncludingGroupBrackets(outerName,
                                                     system) + ldata->instrumentNameOffset();
        } else if (staffNameAlign == AlignH::HCENTER) {
            double sumWidth = innerName->width()
                              + nameWidthIncludingGroupBrackets(outerName,
                                                                system)
                              + ldata->instrumentNameOffset();
            double move = 0.5 * (ldata->firstColumnWidth() - innerName->width());
            return sumWidth + move;
        } else {
            return innerName->width() + ldata->instrumentNameOffset()
                   + nameWidthIncludingGroupBrackets(outerName, system);
        }
    };
    ...
    for (InstrumentName* groupName : groupNames) {
        ...
        if (ldata->useLongNames()
            && system->visiblePartsOfGroup(startStaff, endStaff).size() % 2 == 0) {
            continue;
        }
        for (staff_idx_t idx = startStaff; idx < endStaff; ++idx) {
            if (InstrumentName* n =
                    system->staff(idx)->individualStaffName; n && !n->ldata()->isSkipDraw()) {
                ldata->setTotalNamesWidth(std::max(ldata->totalNamesWidth(),
                                                   sumWidth(groupName, n)));
            }
            if (InstrumentName* n =
                    system->staff(idx)->instrumentName; n && !n->ldata()->isSkipDraw()) {
                ldata->setTotalNamesWidth(std::max(ldata->totalNamesWidth(),
                                                   sumWidth(groupName, n)));
            }
        }
...
void SystemHeaderLayout::setInstrumentNamesVerticalPos(System* system, LayoutContext& ctx)
{
    ...
    for (staff_idx_t staffIdx = 0; staffIdx < ctx.dom().nstaves(); ++staffIdx) {
        ...
        if (InstrumentName* individualName = s->individualStaffName) {
            ...
            individualName->mutldata()->setPosY(0.5
                                                * (staffBBox.top() + staffBBox.bottom()
                                                   - nameBBox.bottom() - nameBBox.top()));
            partsWithIndividualStaffNames.insert(ctx.dom().staff(staffIdx)->part());
        }
    }
    ...
    for (const Part* p : ctx.dom().parts()) {
        ...
        if (visibleStavesCount % 2 == 0) {
            SysStaff* staffAboveMid
                = system->staff(visibleStavesOfPart[visibleStavesCount / 2 - 1]);
    ...
    for (staff_idx_t idx = 0; idx < system->staves().size(); ++idx) {
        ...
        std::vector<Part*> visibleParts = system->visiblePartsOfGroup(idx,
                                                                      groupName->ldata()->endIdxOfGroup());
        ...
        if (instrName) {
            if (stackVertically) {
                ...
                if (visibleStavesCount % 2
                    && system->staff(visibleStaves[visibleStavesCount / 2])->individualStaffName) {
...
void SystemHeaderLayout::setInstrumentNamesHorizontalPos(System* system)
{
    ...
    InstrumentNamesAlign align = system->style().styleV(
        ldata->useLongNames() ? Sid::instrumentNamesAlignLong : Sid::instrumentNamesAlignShort).
                                 value<InstrumentNamesAlign>();

    auto placeFirstColumnName = [&](InstrumentName* name) {
        const RectF& bbox = name->ldata()->bbox();
        if (align == InstrumentNamesAlign::CENTER_CENTER) {
            double groupBracketsWidth = ldata->groupBracketsWidthAtStaffIdx(
                name->effectiveStaffIdx());
            name->mutldata()->setPosX(
                0.5 * totalNamesWidth - 0.5 * (bbox.right() + bbox.left()) + groupBracketsWidth);
        } else {
            ...
            case AlignH::LEFT:
                ...
                break;
            case AlignH::HCENTER:
                name->mutldata()->setPosX(totalNamesWidth - 0.5 * firstColumnWidth - 0.5
    ...
    for (const SysStaff* s : system->staves()) {
        ...
        if (InstrumentName* n =
                s->instrumentName;
            n && n->ldata()->column() == 0 && n->effectiveStaffIdx() != muse::nidx) {
            placeFirstColumnName(n);
        }
    }
    ...
    auto placeSecondColumnName = [&](InstrumentName* name, staff_idx_t staffIdx) {
        ...
        if (align == InstrumentNamesAlign::CENTER_CENTER) {
            name->mutldata()->setPosX(
                0.5 * totalNamesWidth - 0.5 * (bbox.right() + bbox.left()) + 0.5
                * groupBracketsWidth);
            return;
        }

        if (align == InstrumentNamesAlign::CENTER_RIGHT) {
            name->mutldata()->setPosX(0.5 * ldata->secondColumnWidth() - 0.5
                                      * (bbox.right() + bbox.left()) + 0.5 * groupBracketsWidth);
            return;
        }
        ...
        if (name->instrumentNameRole() == InstrumentNameRole::PART) {
            ...
            if (InstrumentName* staffName = system->staff(centerStaff)->individualStaffName) {
                name->mutldata()->setPosX(
                    staffName->x() + staffName->ldata()->bbox().left() - bbox.right()
                    - ldata->instrumentNameOffset());
            } else {
                name->mutldata()->setPosX(totalNamesWidth - bbox.right());
            }
        ...
        std::vector<Part*> visiblePartsOfGroup = system->visiblePartsOfGroup(staffIdx,
                                                                             name->ldata()->endIdxOfGroup());
        ...
        if (InstrumentName* instrName
                = system->staff(*centerPart->staveIdxList().begin())->instrumentName) {
            name->mutldata()->setPosX(
                instrName->x() + instrName->ldata()->bbox().left() - bbox.right()
                - ldata->instrumentNameOffset());
            return;
        }
    ...
    for (staff_idx_t staffIdx = 0; staffIdx < system->staves().size(); ++staffIdx) {
        ...
        if (instrName && instrName->effectiveStaffIdx() != muse::nidx
            && instrName->ldata()->column() > 0) {
...
void SystemHeaderLayout::setGroupBracketsHorizontalPos(System* system)
{
    ...
    bool intoMargin = style.styleB(Sid::groupBracketHangTextIntoMargin)
                      && style.styleV(Sid::groupBracketTextOrientation).value<Orientation>()
                      == Orientation::VERTICAL;
    ...
    for (Bracket* b : groupBrackets) {
        ...
        for (Bracket* bb : groupBrackets) {
            ...
            if (bb->column() > b->column() && bb->intersects(b)) {
                b->mutldata()->setPosX(
                    bb->x() + bb->ldata()->bbox().right() - b->ldata()->bbox().left()
                    + bracketsDist);
            }
        }
    }
...
InstrumentName* SystemHeaderLayout::updateName(System* system, staff_idx_t staffIdx,
                                               LayoutContext& ctx, const String& name,
                                               InstrumentNameType type, InstrumentNameRole role)
{
    ...
    InstrumentName* iname = role == InstrumentNameRole::GROUP ? sysStaff->groupName
                            : role
                            == InstrumentNameRole::PART ? sysStaff->instrumentName : sysStaff->
                            individualStaffName;
    if (name.empty()) {
        if (iname) {
...
String SystemHeaderLayout::formattedInstrumentName(System* system, Part* part, const Fraction& tick)
{
    ...
    showTranspo &= style.styleB(
        longNames ? Sid::instrumentNamesShowTranspositionLong : Sid::instrumentNamesShowTranspositionShort);
    ...
    InstrumentNamesFormat nameFormat
        = style.styleV(longNames ? Sid::instrumentNamesFormatLong : Sid::instrumentNamesFormatShort)
          .value<InstrumentNamesFormat>();
    ...
    case InstrumentNamesFormat::NAME_IN_TRANSP_NUM:
        return instrName + u" " + in + u" " + transposition
               + (number.empty() ? String() : u" " + number);
    case InstrumentNamesFormat::NAME_NUM_IN_TRANSP:
        return instrName + (number.empty() ? String() : u" " + number) + u" " + in + u" "
               + transposition;
    case InstrumentNamesFormat::TRANSP_NAME_NUM:
        return transposition + u" " + instrName + (number.empty() ? String() : u" " + number);
    default:
    {
        String result = style.styleSt(
            longNames ? Sid::instrumentNamesCustomFormatLong : Sid::instrumentNamesCustomFormatShort);
...
String& SystemHeaderLayout::resolveTokens(String& str, const String& name,
                                          const String& transposition, const String& number)
{
...
bool SystemHeaderLayout::showNames(LayoutContext& ctx)
{
    ...
    if (ctx.conf().styleB(Sid::hideInstrumentNameIfOneInstrument)
        && ctx.dom().visiblePartCount() <= 1) {
        return false;
    }
...
void SystemHeaderLayout::setInstrumentNames(System* system, LayoutContext& ctx, bool longName,
                                            Fraction tick)
{
    if (system->vbox()) {
        return;
    ...
    for (size_t staffIdx = 0; staffIdx < system->staves().size(); /*empty*/) {
        ...
        for (size_t idxInPart = 0; idxInPart < partNstaves; ++idxInPart) {
            ...
            if (staff->show()) {
                staffName
                    = longName ? staff->individualStaffNameLong(tick) : staff->
                      individualStaffNameShort(
                          tick);
            }
...
void SystemHeaderLayout::updateGroupNames(System* system, LayoutContext& ctx, const Fraction& tick)
{
    ...
    InstrumentNameType type
        = system->ldata()->useLongNames() ? InstrumentNameType::LONG : InstrumentNameType::SHORT;
    ...
    for (staff_idx_t startOfGroup = 0; startOfGroup < system->staves().size();) {
        ...
        while (endOfGroup < system->staves().size()) {
            ...
            if (nextPart != curPart
                && (nextInstrument->id() != curInstrument->id() || !nextLabel.allowGroupName())) {
                break;
            }
            if (nextLabel.useCustomGroupName()
                && (nextLabel.customNameLongGroup() != curLabel.customNameLongGroup()
                    || nextLabel.customNameShortGroup()
                    != curLabel.customNameShortGroup())) {
                break;
            }

            if (type == InstrumentNameType::LONG
                && system->visibleStavesOfPart(nextPart).size() == 0) {
                // NOTE: We can only do this for long-name systems because they don't need to have
                // the left barline aligned with the other systems across the page.
        ...
        if (partsInThisGroup.size() > 1 && useGroupNames(curInstrument->group())) {
            ...
            InstrumentName* groupName = updateName(system, startOfGroup, ctx, name, type,
                                                   InstrumentNameRole::GROUP);
