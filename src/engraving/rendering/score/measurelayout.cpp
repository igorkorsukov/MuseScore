...
void MeasureLayout::layout2(Measure* item, LayoutContext& ctx)
{
    ...
    auto layoutCrossStaffTies
        = [](Chord* chord, Measure* item, const Fraction& stick, LayoutContext& ctx) ->void {
...
void MeasureLayout::createMMRest(LayoutContext& ctx, Measure* firstMeasure, Measure* lastMeasure,
                                 const Fraction& len)
{
    ...
    //
    Segment* lastMeasureEndBarlineSeg = lastMeasure->findSegmentR(SegmentType::EndBarLine,
                                                                  lastMeasure->ticks());
    if (lastMeasureEndBarlineSeg) {
        Segment* mmrEndBarlineSeg = mmrMeasure->undoGetSegmentR(SegmentType::EndBarLine,
                                                                mmrMeasure->ticks());
        for (size_t staffIdx = 0; staffIdx < ctx.dom().nstaves(); ++staffIdx) {
            ...
            if (e) {
                ...
                if (!mmrEndBarlineSeg->element(staffIdx * VOICES)) {
                    ...
                    BarLine* mmrEndBarline
                        = toBarLine(mmrEndBarlineSeg->element(staffIdx * VOICES));
                    ...
                    if (mmrEndBarline->barLineType() != lastMeasureEndBarline->barLineType()) {
                        ...
                        ctx.mutDom().undo(new ChangeProperty(mmrEndBarline, Pid::GENERATED,
                                                             generated, PropertyFlags::NOSTYLE));
                    }
                }
            }
        }
    ...
    //
    Segment* lastMeasureClefSeg = lastMeasure->findSegmentR(
        SegmentType::Clef | SegmentType::HeaderClef,
        lastMeasure->ticks());
    if (lastMeasureClefSeg) {
        Segment* mmrClefSeg = mmrMeasure->undoGetSegment(
            lastMeasureClefSeg->segmentType(), lastMeasure->endTick());
        for (size_t staffIdx = 0; staffIdx < ctx.dom().nstaves(); ++staffIdx) {
            ...
            if (e && e->isClef()) {
                ...
                if (!mmrClefSeg->element(track)) {
                    Clef* mmrClef
                        = lastMeasureClef->generated() ? lastMeasureClef->clone() : toClef(
    ...
    if (underlyingSeg) {
        ...
        for (staff_idx_t staffIdx = 0; staffIdx < ctx.dom().nstaves(); ++staffIdx) {
            ...
            if (underlyingTimeSig) {
                ...
                if (!mmrTimeSig) {
                    mmrTimeSig
                        = underlyingTimeSig->generated() ? underlyingTimeSig->clone() : toTimeSig(
    ...
    if (underlyingSeg) {
        ...
        for (size_t staffIdx = 0; staffIdx < ctx.dom().nstaves(); ++staffIdx) {
            ...
            if (underlyingKeySig) {
                ...
                if (!mmrKeySig) {
                    mmrKeySig
                        = underlyingKeySig->generated() ? underlyingKeySig->clone() : toKeySig(
                    ...
                    if (!(mmrKeySig->keySigEvent() == underlyingKeySig->keySigEvent())) {
                        ...
                        ctx.mutDom().undo(new ChangeKeySig(mmrKeySig,
                                                           underlyingKeySig->keySigEvent(),
                                                           mmrKeySig->showCourtesy(),
                                                           addKey));
                    }
                }
            }
...
void MeasureLayout::cloneAnnotationsToMMRest(Segment* underlyingSeg, Segment* mmrSeg,
                                             LayoutContext& ctx)
{
    if (!underlyingSeg) {
        return;
            ...
            for (track_idx_t track = 0; track < tracks; ++track) {
                ...
                if (s->element(track)) {
                    if (!s->element(track)->isRest()) {
                        ...
                        bool isNonEmptyIrregular = m->isIrregular()
                                                   && !toRest(s->element(track))->isFullMeasureRest();
...
static bool breakMultiMeasureRest(const LayoutContext& ctx, Measure* m)
{
    ...
    for (auto i : sl) {
        ...
        if (muse::contains(breakSpannerTypes,
                           s->type()) && (spannerStartsInside || spannerEndsInside)) {
            return true;
        }
    }
    ...
    if (prevMeas) {
        auto prevMeasSpanners = ctx.dom().spannerMap().findOverlapping(
            prevMeas->tick().ticks(), prevMeas->endTick().ticks());
        ...
        if (s) {
            for (size_t staffIdx = 0; staffIdx < ctx.dom().nstaves(); ++staffIdx) {
                ...
                if (bl) {
                    ...
                    if (t != BarLineType::NORMAL && t != BarLineType::BROKEN
                        && t != BarLineType::DOTTED && !bl->generated()) {
                        return true;
                    } else {
                        break;
                    }
...
void MeasureLayout::moveToNextMeasure(LayoutContext& ctx)
{
    ...
    if (!state.curMeasure()) {
        state.setNextMeasure(
            ctx.conf().isShowVBox() ? ctx.mutDom().first() : ctx.mutDom().firstMeasure());
    } else {
        MeasureBase* m
            = ctx.conf().isShowVBox() ? state.curMeasure()->next() : state.curMeasure()->nextMeasure();
    ...
    if (ctx.conf().styleB(Sid::createMultiMeasureRests)) {
        ...
        while (validMMRestMeasure(ctx, measureToBeChecked)) {
            ...
            if (measureToBeChecked != firstMeasure) {
                int measureNumber = adjustMeasureNumber(measureToBeChecked,
                                                        ctx.state().measureNumber());
                ctx.mutState().setMeasureNumber(measureNumber);
            }
            ++n;
            len += measureToBeChecked->ticks();
            lastMeasure = measureToBeChecked;
            MeasureBase* nextMeasureBase
                = ctx.conf().isShowVBox() ? measureToBeChecked->next() : measureToBeChecked->
                  nextMeasure();
        ...
        if (n >= ctx.conf().styleI(Sid::minEmptyMeasures)) {
            ...
            ctx.mutState().setNextMeasure(
                ctx.conf().isShowVBox() ? lastMeasure->next() : lastMeasure->nextMeasure());
        } else {
        ...
        if (firstMeasure->mmRestCount() > 0) {
            LOGD("mmrest: measureNumber %d += %d",
                 ctx.state().measureNumber(), firstMeasure->mmRestCount());
...
void MeasureLayout::layoutMeasure(MeasureBase* currentMB, LayoutContext& ctx)
{
    ...
    if (ctx.conf().isLinearMode()
        && (measure->tick() < ctx.state().startTick() || measure->tick() > ctx.state().endTick())) {
        return;
    }
...
void MeasureLayout::layoutStaffLines(Measure* m, LayoutContext& ctx)
{
    ...
    for (const MStaff* ms : m->mstaves()) {
        if (m->isCutawayClef(staffIdx)
            && (ctx.dom().staff(staffIdx)->cutaway() || !m->visible(staffIdx))) {
            ...
            double partialWidth = clefSeg
                                  ? m->width() - clefSeg->x() + clefSeg->minLeft()
                                  + ctx.conf().styleAbsolute(Sid::clefLeftMargin)
                                  * staffMag
                                  : 0.0;
            layoutPartialWidth(ms->lines(), ctx, m->width(),
                               partialWidth / (m->spatium() * staffMag), true);
        } else {
            // normal staff lines
...
void MeasureLayout::layoutPlayCountText(Measure* m, LayoutContext& ctx)
{
    ...
    for (staff_idx_t staffIdx = 0; staffIdx < score->nstaves(); ++staffIdx) {
        ...
        PlayCountText* playCount
            = endBarSeg ? toPlayCountText(endBarSeg->findAnnotation(ElementType::PLAY_COUNT_TEXT,
                                                                    track, track)) : nullptr;
        if (!playCount) {
            continue;
        }
        ...
        String defaultText = TConv::translatedUserName(ctx.conf().styleV(
...
MeasureLayout::MeasureStartEndPos MeasureLayout::getMeasureStartEndPos(const Measure* measure,
                                                                       ...
                                                                       const LayoutContext& ctx)
{
    ...
    for (s2 = firstCrSeg->nextActive(); s2; s2 = s2->nextActive()) {
        ...
        if (!modernMMRest
            && (s2->isClefRepeatAnnounceType()
                || (s2->isTimeSigRepeatAnnounceType() && !s2->hasTimeSigAboveStaves())
                || s2->isKeySigRepeatAnnounceType() || s2->isEndBarLineType())
    ...
    bool headerException = measure->header() && firstCrSeg->prev()
                           && !firstCrSeg->prev()->isStartRepeatBarLineType()
...
void MeasureLayout::layoutMeasureElements(Measure* m, LayoutContext& ctx)
{
    ...
    for (Segment& s : m->segments()) {
        ...
        for (EngravingItem* e : s.elist()) {
            ...
            if ((e->isRest() && toRest(e)->isFullMeasureRest()) || e->isMMRest()
                || e->isMeasureRepeat()) {
                //
                ...
                const MeasureStartEndPos measureStartEnd = getMeasureStartEndPos(m, &s, staffIdx,
                                                                                 e->isMMRest(), modernMMRest,
                                                                                 ctx);
                ...
                if (e->isMMRest()) {
                    ...
                    if (toMeasureRepeat(e)->numMeasures() == 4
                        && ctx.conf().styleB(Sid::fourMeasureRepeatShowExtenders)) {
                        TLayout::layoutMeasureRepeatExtender(toMeasureRepeat(e), toMeasureRepeat(
                                                                 e)->mutldata(), ctx);
                    }
                } else {
                    ...
                    if (shEL) {
                        e->mutldata()->setPosX(
                            (x2 - x1 - shEL->width()) * .5 + x1 - s.x() - shEL->x());
                    }
                }
...
void MeasureLayout::createEndBarLines(Measure* m, bool isLastMeasureInSystem, LayoutContext& ctx)
{
    ...
    if (nextMeasure && !sectionBreakHideCourtesy) {
        ...
        const bool showCourtesyKeySig
            = (isLastMeasureInSystem && ctx.conf().styleB(Sid::genCourtesyKeysig))
              || (m->repeatJump()
        ...
        bool showCourtesyTimeSig
            = (isLastMeasureInSystem && ctx.conf().styleB(Sid::genCourtesyTimesig))
              || (m->repeatJump()
                  && ctx.conf().styleB(Sid::showCourtesiesOtherJumps));
    ...
    bool nextMeasRepeat = nextMeasure && nextMeasure->repeatStart() && !m->repeatEnd()
                          && !isLastMeasureInSystem
                          && m->next() == nextMeasure;

    bool barlineBeforeRepeatSigs = ctx.conf().styleB(Sid::barlineBeforeSigChange)
                                   && (hasTimeSig || hasKeySig);
    ...
    if (nextMeasRepeat && !barlineBeforeRepeatSigs) {
        ...
        const bool isRepeatSigBarline = ctx.conf().styleB(Sid::barlineBeforeSigChange)
                                        && nextMeasRepeat;

        const int keySigBarlineMode = ctx.conf().styleI(Sid::keySigCourtesyBarlineMode);
        const bool beforeKSCourtesy = keySigBarlineMode
                                      == int(CourtesyBarlineMode::DOUBLE_BEFORE_COURTESY)
                                      && m->hasCourtesyKeySig();
        const bool alwaysDoubleKS = keySigBarlineMode == int(CourtesyBarlineMode::ALWAYS_DOUBLE)
                                    && hasKeySig;
        const bool beforeRepeatKS = isRepeatSigBarline && hasKeySig && ctx.conf().styleB(
            Sid::doubleBarlineBeforeKeySig);
        ...
        const bool beforeTSCourtesy = timeSigBarlineMode
                                      == int(CourtesyBarlineMode::DOUBLE_BEFORE_COURTESY)
                                      && hasCourtesyTimeSig;
        const bool alwaysDoubleTS = timeSigBarlineMode == int(CourtesyBarlineMode::ALWAYS_DOUBLE)
                                    && hasTimeSig;
        const bool beforeRepeatTS = isRepeatSigBarline && hasTimeSig && ctx.conf().styleB(
            Sid::doubleBarlineBeforeTimeSig);
...
void MeasureLayout::setClefSegVisibility(Measure* m, bool isLastMeasureInSystem, LayoutContext& ctx)
{
    ...
    for (staff_idx_t staffIdx = 0; staffIdx < nstaves; ++staffIdx) {
        ...
        const bool lastMeasure = isLastMeasureInSystem
                                 || (nextMeasure ? !(m->next() == nextMeasure) : true);
    ...
    if (visibleClef) {                       // there is at least one visible clef in the clef segment
        ...
        LOGD("Clef Segment without Clef elements at tick %d/%d",
             clefSeg->tick().numerator(), clefSeg->tick().denominator());
        clefSeg->setVisible(false);
        return;
    }
}

void MeasureLayout::setCourtesyTimeSig(Measure* m, const Fraction& refSigTick,
                                       const Fraction& courtesySigTick,
                                       const SegmentType courtesySegType, LayoutContext& ctx)
{
    ...
    const Segment* prevCourtesySegment
        = prevMeasure ? prevMeasure->findSegmentR(SegmentType::TimeSigRepeatAnnounce,
                                                  prevMeasure->ticks()) : nullptr;
    ...
    const bool checkPrevMeasure = isContinuationCourtesy && prevMeasure
                                  && ((prevMeasure->repeatEnd()
                                       && ctx.conf().styleB(Sid::changesBeforeBarlineRepeats))
    ...
    tsSegAtCourtesyTick = !tsSegAtCourtesyTick && checkPrevMeasure ? prevMeasure->findSegmentR(
        SegmentType::TimeSig,
        courtesySigTick
        - prevMeasure->tick())
                          : tsSegAtCourtesyTick;
    ...
    for (track_idx_t track = 0; track < nstaves * VOICES; track += VOICES) {
        ...
        const bool needsCourtesy = isContinuationCourtesy ? shouldShowContCourtesy
                                   && prevCourtesySegment && prevCourtesySegment->element(
        ...
        const bool hasSigAtTick = tsSegAtCourtesyTick && tsSegAtCourtesyTick->enabled()
                                  && tsSegAtCourtesyTick->element(track);
        ...
        const bool show = actualShowCourtesy && needsCourtesy && !hasSigAtTick && ctx.conf().styleB(
    ...
    if (courtesySigSeg
        && ((!isContinuationCourtesy && !m->hasCourtesyTimeSig())
            || !ctx.conf().styleB(Sid::genCourtesyTimesig))) {
...
void MeasureLayout::setCourtesyKeySig(Measure* m, const Fraction& refSigTick,
                                      const Fraction& courtesySigTick,
                                      const SegmentType courtesySegType, LayoutContext& ctx)
{
    // Find original element
    ...
    const Segment* prevCourtesySegment
        = prevMeasure ? prevMeasure->findSegmentR(SegmentType::KeySigRepeatAnnounce,
                                                  prevMeasure->ticks()) : nullptr;
    ...
    ksSegAtCourtesyTick = !ksSegAtCourtesyTick && checkPrevMeasure ? prevMeasure->findSegmentR(
        SegmentType::KeySig,
        courtesySigTick
        - prevMeasure->tick())
                          : ksSegAtCourtesyTick;
    ...
    for (track_idx_t track = 0; track < nstaves * VOICES; track += VOICES) {
        ...
        const Segment* actualKeySigSeg
            = refMeasure ? refMeasure->findSegmentR(SegmentType::KeySig,
                                                    refSigElementTick
                                                    - refMeasure->tick()) : nullptr;
        ...
        const bool staffIsPitchedAtNextMeas = ctx.dom().lastMeasureMM() == m
                                              || (m->nextMeasureMM()
                                                  && staff->isPitchedStaff(
                                                      m->nextMeasureMM()->tick()));
        // If there is a real key sig at this tick (in this bar or the previous), don't create a courtesy
        const bool hasSigAtTick = ksSegAtCourtesyTick && ksSegAtCourtesyTick->enabled()
                                  && ksSegAtCourtesyTick->element(track);
        // Only show courtesy if its real signature has courtesies enabled
    ...
    if (courtesySigSeg
        && ((!isContinuationCourtesy && !m->hasCourtesyKeySig())
            || !ctx.conf().styleB(Sid::genCourtesyKeysig))) {
        // Whole segment shouldn't be shown, remove any existing courtesy signatures
        if (!isContinuationCourtesy) {
            m->setHasCourtesyKeySig(false);
        }
...
void MeasureLayout::setCourtesyClef(Measure* m, const Fraction& refClefTick,
                                    const Fraction& courtesyClefTick,
                                    const SegmentType courtesySegType, LayoutContext& ctx)
{
    // Find original element
    const size_t nstaves = ctx.dom().nstaves();
    ...
    const Segment* prevCourtesySegment
        = prevMeasure ? prevMeasure->findSegmentR(SegmentType::ClefRepeatAnnounce,
                                                  prevMeasure->ticks()) : nullptr;
    const Segment* clefSegAtCourtesyTick = m->findSegmentR(
        SegmentType::Clef | SegmentType::HeaderClef, courtesyClefRTick);
    clefSegAtCourtesyTick
        = !clefSegAtCourtesyTick
          && prevMeasure ? prevMeasure->findSegmentR(SegmentType::KeySig,
                                                     courtesyClefTick
                                                     - prevMeasure->tick()) : clefSegAtCourtesyTick;
    ...
    for (track_idx_t track = 0; track < nstaves * VOICES; track += VOICES) {
        ...
        if (!actualClefSeg && refMeasure && refMeasure->prevMeasureMM()) {
            ...
            actualClefSeg
                = refPrevMeasure->findSegmentR(SegmentType::Clef | SegmentType::HeaderClef,
                                               refClefElementTick - refPrevMeasure->tick());
        }
        ...
        const bool hasClefAtTick = clefSegAtCourtesyTick && clefSegAtCourtesyTick->enabled()
                                   && clefSegAtCourtesyTick->element(track);
        ...
        const bool show = actualShowCourtesy && needsCourtesy && !hasClefAtTick
                          && ctx.conf().styleB(Sid::genCourtesyClef);
    ...
    if (courtesyClefSeg
        && (!ctx.conf().styleB(Sid::genCourtesyClef)
            || (!isContinuationCourtesy && !m->hasCourtesyClef()))) {
        // Whole segment shouldn't be shown, remove any existing courtesy signatures
        if (!isContinuationCourtesy) {
...
void MeasureLayout::addRepeatCourtesyParentheses(Measure* m, const bool continuation,
                                                 LayoutContext& ctx)
{
    const SegmentType tsSegType
        = continuation ? SegmentType::TimeSigStartRepeatAnnounce : SegmentType::
          TimeSigRepeatAnnounce;
    const SegmentType ksSegType
        = continuation ? SegmentType::KeySigStartRepeatAnnounce : SegmentType::KeySigRepeatAnnounce;
    const SegmentType clefSegType
        = continuation ? SegmentType::ClefStartRepeatAnnounce : SegmentType::ClefRepeatAnnounce;
    ...
    auto timeSigShouldHaveOwnParentheses
        = [&](const Segment* seg, const track_idx_t track) -> bool {
        ...
        return ctx.conf().styleV(Sid::timeSigPlacement).value<TimeSigPlacement>()
               != TimeSigPlacement::NORMAL;
    };

    auto createParenthesesForSegments
        = [&](Segment* leftSeg, Segment* rightSeg, track_idx_t track) -> void {
        double top = DBL_MAX;
        ...
        if (leftItem) {
            leftItem->setParenthesesMode(
                leftItem->rightParen() ? ParenthesesMode::BOTH : ParenthesesMode::LEFT, true, true);
            leftParen = leftItem->leftParen();
        }
        Parenthesis* rightParen = nullptr;
        if (rightItem) {
            rightItem->setParenthesesMode(
                rightItem->leftParen() ? ParenthesesMode::BOTH : ParenthesesMode::RIGHT, true,
                true);
            rightParen = rightItem->rightParen();
        }
        bool needsBigTimeSigAdjust = leftSeg && rightSeg && leftSeg == rightSeg && leftSeg->isType(
            SegmentType::TimeSigType)
                                     && ctx.conf().styleV(Sid::timeSigPlacement).value<TimeSigPlacement>()
                                     != TimeSigPlacement::NORMAL;
        if ((ctx.conf().styleB(Sid::smallParens) || needsBigTimeSigAdjust) && leftParen
            && rightParen) {
            ...
            rightParen->mutldata()->midPointThickness.set_value(
                height / 60 * rightParen->ldata()->mag());                                                  // 0.1sp for a height of 6sp
            leftParen->mutldata()->startY.set_value(top);
            leftParen->mutldata()->height.set_value(height);
            leftParen->mutldata()->midPointThickness.set_value(
                height / 60 * leftParen->ldata()->mag());                                                 // 0.1sp for a height of 6sp
            ...
            if (leftParen) {
                ...
                leftParen->mutldata()->height.set_value(staff->staffHeight(
                                                            tick) + 2 * spatium * leftParen->mag());
                leftParen->mutldata()->midPointThickness.set_value(
                    leftParen->ldata()->height / 60 * leftParen->ldata()->mag());                                                 // 0.1sp for a height of 6sp
            }
            if (rightParen) {
                rightParen->mutldata()->startY.set_value(-spatium);
                rightParen->mutldata()->height.set_value(staff->staffHeight(
                                                             tick) + 2 * spatium
                                                         * rightParen->mag());
                rightParen->mutldata()->midPointThickness.set_value(
                    rightParen->ldata()->height / 60 * rightParen->ldata()->mag());                                                  // 0.1sp for a height of 6sp
            }
        }
...
void MeasureLayout::removeRepeatCourtesyParenthesesMeasure(Measure* m, const bool continuation,
                                                           LayoutContext& ctx)
{
    const SegmentType tsSegType
        = continuation ? SegmentType::TimeSigStartRepeatAnnounce : SegmentType::
...
{
    const bool showCourtesyRepeats = m->repeatEnd()
                                     && ctx.conf().styleB(Sid::showCourtesiesRepeats);
    const bool showCourtesyOtherJumps = m->repeatJump() && ctx.conf().styleB(
        Sid::showCourtesiesOtherJumps);
    ...
    const bool courtesiesAfterCancellingRepeats = m->prevMeasure() && m->prevMeasure()->repeatEnd()
        ...
        Sid::showCourtesiesAfterCancellingRepeats)
                                                  && ctx.conf().styleB(Sid::showCourtesiesRepeats);
    const bool courtesiesAfterCancellingOtherJumps = m->prevMeasure()
                                                     && m->prevMeasure()->repeatJump()
                                                     && ctx.conf().styleB(
...
void MeasureLayout::addRepeatCourtesies(Measure* m, LayoutContext& ctx)
{
    ...
    for (Measure* repeatStartMeasure : measures) {
        ...
        const LayoutBreak* sectionBreak
        ...
        setCourtesyClef(m, repeatStartMeasure->tick(),
                        m->endTick(), SegmentType::ClefRepeatAnnounce, ctx);
        setCourtesyKeySig(m, repeatStartMeasure->tick(),
                          m->endTick(), SegmentType::KeySigRepeatAnnounce, ctx);
        setCourtesyTimeSig(m, repeatStartMeasure->tick(),
                           m->endTick(), SegmentType::TimeSigRepeatAnnounce, ctx);
...
void MeasureLayout::removeRepeatCourtesies(Measure* m)
{
    for (Segment* seg = m->last(); seg != m->first(); seg = seg->prev()) {
        ...
        if (!seg->isType(SegmentType::ClefRepeatAnnounce | SegmentType::TimeSigRepeatAnnounce
...
void MeasureLayout::removeRepeatContinuationCourtesies(Measure* m)
{
    for (Segment* seg = m->first(); seg != m->last(); seg = seg->next()) {
        ...
        if (!seg->isType(SegmentType::ClefStartRepeatAnnounce
...
Segment* MeasureLayout::addHeaderClef(Measure* m, bool isFirstClef, const Staff* staff,
                                      LayoutContext& ctx)
{
    ...
    const bool hideClef
        = staffType->isTabStaff() ? ctx.conf().styleB(Sid::hideTabClefAfterFirst) : !ctx.conf().
          styleB(
              Sid::genClef);
...
Segment* MeasureLayout::addHeaderKeySig(Measure* m, bool isFirstKeysig, const Staff* staff,
                                        LayoutContext& ctx)
{
    const staff_idx_t staffIdx = staff->idx();
    const track_idx_t track = staffIdx * VOICES;
    ...
    for (const Staff* staff : ctx.dom().staves()) {
        ...
        if (!isFirstSystem) {
            ...
            searchMeasure = searchMeasure->hasMMRest()
                            && ctx.conf().styleB(Sid::createMultiMeasureRests) ? searchMeasure->
                            mmRest() : searchMeasure;
            while (searchMeasure->tick() < m->tick() && (isFirstClef || isFirstKeySig)) {
                ...
                if (isFirstClef && searchMeasure->tick() >= clefTick) {
                    ...
                    Segment* clefSeg = searchMeasure->findFirstR(
                        SegmentType::Clef | SegmentType::HeaderClef, Fraction(0, 1));
                ...
                if (isFirstKeySig && searchMeasure->tick() >= keySigTick) {
                    const Segment* ksSeg = searchMeasure->findSegment(SegmentType::KeySig,
                                                                      searchMeasure->tick());
...
void MeasureLayout::addSystemTrailer(Measure* m, Measure* nm, LayoutContext& ctx)
{
    ...
    // check if it has court. sig. turned off
    if (nm && ctx.conf().styleB(Sid::genCourtesyTimesig) && !systemBreakHideCourtesy
        && !ctx.conf().isFloatMode()) {
...
void MeasureLayout::createSystemBeginBarLine(Measure* m, LayoutContext& ctx)
{
    ...
    if ((n > 1 && ctx.conf().styleB(Sid::startBarlineMultiple))
        || (n == 1
            && (ctx.conf().styleB(Sid::startBarlineSingle) || m->system()->brackets().size()))) {
        if (!s) {
...
//            }
    ...
    for (Segment& s : m->segments()) {
        ...
        for (EngravingItem* e : s.elist()) {
            ...
            staff_idx_t staffIdx = e->staffIdx();
            if (t == ElementType::MEASURE_REPEAT
                || (t == ElementType::REST && (m->isMMRest() || toRest(e)->isFullMeasureRest()))) {
                //
                // element has to be centered in free space
                //    x1 - left measure position of free space
                //    x2 - right measure position of free space
                ...
                if (m->isMMRest()) {
                    ...
                    //
                    e->mutldata()->setPosX(
                        (x2 - x1 - e->width()) * .5 + x1 - s.x() - e->ldata()->bbox().x());
                    s.createShape(staffIdx);  // DEBUG
                }
...
void MeasureLayout::layoutTimeTickAnchors(Measure* m, LayoutContext& ctx)
{
    ...
    for (Segment& segment : m->segments()) {
        ...
        Segment* nextSeg = m->findSegmentR(SegmentType::ChordRest,
                                           refCRSeg->rtick() + refCRSeg->ticks());
        if (!(nextSeg && nextSeg->isActive())) {
            nextSeg = m->findSegmentR(SegmentType::BarLineType,
                                      refCRSeg->rtick() + refCRSeg->ticks());
        }
...
void MeasureLayout::layoutPartialWidth(StaffLines* lines, LayoutContext& ctx, double w,
                                       double wPartial, bool alignRight)
{
