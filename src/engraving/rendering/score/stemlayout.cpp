...
double StemLayout::calcDefaultStemLength(Chord* item, const LayoutContext& ctx)
{
    ...
    double extraHeight
        = (ldata->up ? item->upNote()->stemUpSE().y() : item->downNote()->stemDownNW().y())
          / item->intrinsicMag()
          / spatium;
    int shortestStem
        = style.styleB(Sid::useWideBeams) ? 12 : (style.styleD(Sid::shortestStem) + std::abs(
                                                      extraHeight)) * 4;
    ...
    if (ldata->up) {
        ...
        if (stemEndPositionMag <= -shortStemStart) {
            int reduction
                = maxReduction(item, ctx,
                               std::abs((int)floor(stemEndPositionMag) + shortStemStart));
        ...
        if (stemEndPositionMag >= downShortStemStart) {
            int reduction
                = maxReduction(item, ctx,
                               std::abs((int)ceil(stemEndPositionMag) - downShortStemStart));
            idealStemLength = std::max(idealStemLength - reduction, shortestStem);
        } else if (stemEndPosition < middleLine) {
            // this case will be taken care of below; even if we were to adjust here we'd have
            // to adjust again later if the line spacing != 1.0 or if _relativeMag != 1.0
    ...
    double finalStemLength = (chordHeight / 4.0 * spatium)
                             + ((stemLength / 4.0 * spatium) * item->intrinsicMag());
...
int StemLayout::minStaffOverlap(bool up, int staffLines, int beamCount, bool hasHook,
                                double beamSpacing, bool useWideBeams,
                                bool isFullSize)
{
    int beamOverlap = 8;
    if (isFullSize) {
...
PointF StemLayout::stemPos(const Chord* item)
{
    ...
    if (staffType && staffType->isTabStaff()) {
        return item->pagePos()
               + rendering::score::StemLayout::tabStemPos(item, staffType) * item->spatium();
    }
...
double StemLayout::tabRestStemPosY(const ChordRest* item, const StaffType* st)
{
    ...
    double delta                                 // displacement for half note stems (if used)
        =   // if half notes have not a short stem OR not a half note => 0
          (st->minimStyle() != TablatureMinimStyle::SHORTER
    ...
    double y
        = (item->up() ? STAFFTYPE_TAB_DEFAULTSTEMPOSY_UP : (st->lines() - 1)
           * st->lineDistance().val() + STAFFTYPE_TAB_DEFAULTSTEMPOSY_DN)
          + delta;
    return y;
}
...
double StemLayout::tabStemLength(const Chord* item, const StaffType* st)
{
    ...
    // if stems beside staff, length is fixed, but take into account shorter half note stems
        ...
        bool shrt = (st->minimStyle() == TablatureMinimStyle::SHORTER)
                    && (item->durationType().type() == DurationType::V_HALF);
        stemLen
            = (st->stemsDown() ? STAFFTYPE_TAB_DEFAULTSTEMLEN_DN : STAFFTYPE_TAB_DEFAULTSTEMLEN_UP)
...
int StemLayout::calcMinStemLength(Chord* item, const LayoutContext& ctx)
{
    ...
    if (item->tremoloSingleChord()) {
        ...
        minStemLength += ceil(
            item->tremoloSingleChord()->minHeight() / item->intrinsicMag() * 4.0
            * buzzRollMultiplier);
        ...
        if (item->hook()) {
            ...
            int hookOffset = floor(
                (item->hook()->height() / item->intrinsicMag() + smuflAnchor) / spatium * 4)
                             - (straightFlags ? 0 : 2);
    ...
    if (item->beam() || item->tremoloTwoChord()) {
        ...
        bool noteInStaff = (ldata->up && noteLine > 0)
                           || (!ldata->up && noteLine < (staffLines - 1) * 2);
