...
void SingleDraw::drawItem(const EngravingItem* item, Painter* painter, const PaintOptions& opt)
{
    ...
    case ElementType::LEDGER_LINE:  draw(item_cast<const LedgerLine*>(item), painter, opt);
        break;
    case ElementType::LET_RING_SEGMENT:     draw(item_cast<const LetRingSegment*>(
                                                     item), painter, opt);
        break;
    ...
    case ElementType::NOTEHEAD:             draw(item_cast<const NoteHead*>(item), painter, opt);
        break;
    case ElementType::NOTELINE_SEGMENT:     draw(item_cast<const NoteLineSegment*>(
                                                     item), painter, opt);
        break;
...
void SingleDraw::draw(const ChordBracket* item, muse::draw::Painter* painter,
...
void SingleDraw::draw(const Note* item, Painter* painter, const PaintOptions& opt)
{
    ...
    if (tablature) {
        ...
        if (item->tieBackNonPartial() && !tab->showBackTied()) {
            if (item->chord()->measure()->system()
                == item->tieBack()->startNote()->chord()->measure()->system()
                && item->el().empty()) {
        ...
        painter->drawText(PointF(startPosX, tab->fretFontYOffset() * item->magS()),
                          item->fretString());
    }
    // NOT tablature
    else {
...
void SingleDraw::draw(const Bend* item, Painter* painter, const PaintOptions& opt)
{
    ...
    Pen pen(item->curColor(opt), _lw, PenStyle::SolidLine, PenCapStyle::RoundCap,
...
void SingleDraw::draw(const FretDiagram* item, Painter* painter, const PaintOptions& opt)
{
    ...
    painter->setPen(pen);
    painter->drawLine(LineF(-ldata->stringLineWidth * .5, 0.0, x2 + ldata->stringLineWidth * .5,
                            0.0));
    ...
    if (item->fretOffset() > 0) {
        ...
        if (item->orientation() == Orientation::VERTICAL) {
            if (item->numPos() == 0) {
                painter->drawText(RectF(-ldata->stringDist * .4, .0, .0,
                                        ldata->fretDist),
                                  muse::draw::AlignVCenter | muse::draw::AlignRight | muse::draw::TextDontClip,
                                  text);
            } else {
                painter->drawText(RectF(x2 + (ldata->stringDist * .4), .0, .0,
                                        ldata->fretDist),
...
void SingleDraw::draw(const Stem* item, Painter* painter, const PaintOptions& opt)
{
    ...
    if (nDots > 0 && !staffType->stemThrough()) {
        ...
        double y     = ((score::StemLayout::STAFFTYPE_TAB_DEFAULTSTEMLEN_DN * 0.2) * sp)
                       * (isUp ? -1.0 : 1.0);
...
void SingleDraw::draw(const TextBlock& textBlock, const TextBase* item,
...
void SingleDraw::draw(const TextFragment& textFragment, const TextBase* item,
                      muse::draw::Painter* painter)
{
...
void SingleDraw::draw(const KeySig* item, Painter* painter, const PaintOptions& opt)
{
    ...
    if (!item->explicitParent() && (item->isAtonal() || item->isCustom())
        && ldata->keySymbols.empty()) {
...
void SingleDraw::draw(const TremoloSingleChord* item, Painter* painter, const PaintOptions& opt)
{
    ...
    // vertical line (stem)
    {
        ...
        Pen pen(item->curColor(opt),
                item->absoluteFromSpatium(item->style().styleS(Sid::stemWidth)));
        painter->setPen(pen);
